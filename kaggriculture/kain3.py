...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money, tm=None):
    ...
    # KAIN-3 = KAIN-2 collider + dairy floor: v5 never counter-scales milk
    # (R86) — commit 6 cows unconditionally; capital-light (mild vs kain1's 12).
    goose_target = max(3, min(6, int(egg_room // 46)))
    cow_target = max(6, min(9, int(milk_room // 30)))
    sheep_target = max(3, min(7, int(wool_room // 30)))
    if day < 2:
        goose_target = 0
        cow_target = 0
        sheep_target = 0
