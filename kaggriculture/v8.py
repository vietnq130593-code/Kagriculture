...
def _build_tasks(tiles, shed, seeds, plan, day, hour, step, inventories, n_units):
            ...
            if kind == "PLANT":
                ...
                if not watered:
                    ...
                    if cd["ongoing"]:
                        ...
                        # v8f: wheat/carrot seedling-survival hạ tier 4 —
                        # tưới hạt non age 0-1 = 0 yield nhưng dính phase-1
                        # nuốt units, bỏ đói tưới WINDOW (s115: d8 harvest
                        # 1/12, thung lũng vốn sâu thêm $800 → thua cả trận)
                        if crop in ("STRAWBERRY", "TOMATO"):
                            tasks.append(_mk(T_WATER_CRIT, x, y, "WATER"))
                            stats["water_crit"] += 1
                        else:
                            tasks.append(_mk(4, x, y, "WATER"))
    ...
    if budget and hour <= 19:
        ...
        _pt = 4 if len(empties) > 6 else T_PLANT
