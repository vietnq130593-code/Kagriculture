...
T_WATER_CRIT = 0     # cây sắp thành cỏ dại (hôm qua không được tưới)
T_SERVICE_URG = 1    # thú sắp bỏ trốn (hôm qua không được ăn)
T_HARVEST_URG = 2    # thu hoạch gấp (tràn chuồng / cây sắp tàn)
T_DELIVER = 2        # đưa thú tới chuồng
T_BUILD_URG = 2      # xây chuồng khi thú đang chờ trong kho
T_SERVICE = 2        # ghép FEED+CARE+COLLECT_FERT cho 1 con thú
T_WATER_YIELD = 3    # tưới trong cửa sổ năng suất
T_BUILD = 4          # xây chuồng (kế hoạch đã tính tiền -> xây ngay)
T_HARVEST_ANIMAL = 4 # thu sản phẩm thú (khẩn cấp vừa)
T_HARVEST = 5        # thu hoạch cây bình thường
T_PLANT = 5          # trồng mới
T_DIG = 5            # cuốc cỏ dại
T_WATER_MAINT = 3    # tưới duy trì theo lịch parity (mỗi 2 ngày)
T_FERTILIZE = 7
...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money):
    ...
    goose_target = 5 if 2 <= day <= 14 else 0
    cow_target = 4 if 6 <= day <= 16 else 0
    sheep_target = 5 if 4 <= day <= 15 else 0
    ...
    for crop in CROPS:
        ...
        if day + cl - 1 > 28:
            continue
        cycles = max(1, (28 - day) // cl)
        upt = cycles * YIELD_PER_CYCLE[crop]
        r = room(crop)
        if crop == "WHEAT":
            r += feed_demand
        if r <= 4:
        ...
        pr = prices.get(crop, base) if prices else base
        ...
        if day <= 14 and money < 4000:  # thiếu tiền mặt -> né cây chậm
            if cl >= 17:
                ...
                score *= 0.85
        entries.append((score, crop, tile_cap))
    entries.sort(key=lambda e: -e[0])

    crop_tiles = {}
    remaining = len(plantable)
    for score, crop, cap in entries:
        share_cap = int(SHARE.get(crop, 0.5) * len(plantable)) if plantable else 0
        take = min(remaining, cap, max(1, share_cap))
        if take > 0:
        ...
        if crop_tiles.get("WHEAT", 0) < 2 and sum(crop_tiles.values()) > 0:
            for c in list(crop_tiles):
                if c != "WHEAT" and crop_tiles[c] > 0:
                    crop_tiles[c] -= 1
                    crop_tiles["WHEAT"] = crop_tiles.get("WHEAT", 0) + 1
                    if crop_tiles[c] <= 0:
                        del crop_tiles[c]
    ...
    if day <= 1 and plantable:
        ...
        if fast < 0.55 * len(plantable):
            need = int(0.55 * len(plantable)) - fast
            for c in list(crop_tiles):
                if c not in ("WHEAT", "CARROT") and need > 0:
                    ...
                    if crop_tiles[c] == 0:
                        del crop_tiles[c]
            if need > 0:
                crop_tiles["CARROT"] = crop_tiles.get("CARROT", 0) + need
...
def _task_still_valid(tk, tiles, board, shed, uinv):
    ...
    if op == "FERTILIZE":
        return isinstance(t, dict) and t.get("kind") == "PLANT" and t.get("crop") in ("WHEAT", "CARROT") \
            and t.get("fertilized_until_day", -1) < 9999
    if op == "DELIVER":
...
def _build_orders(me, shed, seeds, inventories, inv, prices, day, hour, plan,
    ...
    if nq < 4 and bought.get("LAND", 0) < 1:
        ...
        last_day = 20 if nq == 3 else 24
        buffer = int(0.3 * price) + 300
        if day <= last_day and (day <= 1 or owned_empty <= 12 or money >= price * 2.5) \
    ...
    if animals > 0 or plan_animals > 0:
        ...
        if wt < wheat_want and money >= 150:
            need = min(10, wheat_want - wt)
            pw = _price("WHEAT", inv.get("WHEAT", MARKET_I0) - 1)
            afford = int((money * 0.25) // pw) if pw > 0 else 0
            n = min(need, afford)
            if n >= 1 and pw <= 65:
    ...
    if shed:
        for it in PRODUCTS:
            ...
            if n <= 0:
                continue
            eff_frac = min(frac, 0.30) if it in glutted else frac
            thresh = eff_frac * MARKET_PARAMS[it]["base"]
