...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money, tm=None, rg=None):
    ...
    # ---- vE2b LATE-HERD liều nhẹ: +2 bò +1 cừu, chỉ khi lao động còn dư ----
    try:
        _st_n = sum(standing.values())
        if 13 <= day <= 17 and money >= 8000 and water_debt <= 1 \
                and _st_n <= 34 and sum(standing.values()) >= 0:
            animal_cap = min(ANIMAL_CAP, animal_cap + 3)
            cow_target = min(12, int(cow_target) + 2)
            sheep_target = min(10, int(sheep_target) + 1)
    ...
    # ---- vE5: đất mới trồng WHEAT (nuôi pump + volume cuối, KHÔNG đụng dâu) ----
    try:
        if (tm or {}).get("late_land_done") and 15 <= day <= 21:
            _pl = len(plantable)
            if _pl > 0 and room("WHEAT") > -100:
                quotas["WHEAT"] = min(34, quotas.get("WHEAT", 0) + _pl)
    except Exception:
        pass
    if day <= 23:
        if day <= 2:
...
def _build_orders(me, shed, seeds, inventories, inv, prices, day, hour, plan,
    ...
    if land_ready:
        ...
        bought["LAND"] = 1
        money -= lp
    # ---- vE5 LATE-LAND WHEAT: SW d14-17 khi đứng cây ≤30 + tiền ≥ $8k ----
    try:
        if not _STATE.get("late_land_done") and 14 <= day <= 17 \
                and len(unlocked) == 2 and bought.get("LAND", 0) < 1 \
                and not (rg and int(rg.get("F1", 0) or 0) >= 2):
            _st0 = sum(1 for row in tiles for t in row
                       if isinstance(t, dict) and t.get("kind") == "PLANT")
            _wd = int((_STATE.get(("risk", day - 1)) or {}).get("water_crit_late", 0) or 0)
            if money >= 8000 and len(hands) >= 12 and _wd <= 2 and _st0 <= 30:
                orders.append(["BUY_LAND"])
                bought["LAND"] = bought.get("LAND", 0) + 1
                money -= 2000
                _STATE["late_land_done"] = True
                tmx["knobs_late_wheat"] = True
    except Exception:
        pass
