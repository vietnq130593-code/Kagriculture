# kain34 "FILL-EVERYTHING" (Task 34/Phase-1) — kain33 + tối ưu nguồn lực đất.
...
# ---- kain39 "PRICE-BLADE" (Task 34/Phase-2 — đòn tấn công chiến lược):
#   E1 WHEAT-HOLD d6-16: 0.76 -> 1.35 (chỉ bán >= $33.75, dưới gate $38
#       thường của v6): v6 vẫn mua nhưng trả đủ 25-40% đắt hơn; phần dư
#       bán d17+ ở $40-49 (mở van bằng gate giá thay vì thời gian).
#       Mục tiêu kép: +doanh thu wheat + siết margin arbitrage feed v6
#       (R113: v6 mua 600-700u/mùa — $27-36k chi feed).
#   E2 EGG-FORTRESS-8: geese floor 7 -> 8 khi opp_geese <= 6 (R102: kênh
#       trứng T=332 sâu, v6 hard-cap 6 — không đối chiếu được).
# Giữ: mọi thứ của kain38.
...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money, tm=None):
    ...
    if opp_counts["GOOSE"] <= 6 and day >= 4:
        goose_target = max(goose_target, 8)
    ...
    if remaining > 0 and day <= 22:
        _late_cap = 16 if day >= 17 else 8
        extra = min(remaining, _late_cap)
        if feed_demand > 200:
    ...
    # Δ1' FILL-SMART (kain35): d4-13 — hạt dâu không nổi floor thì vẫn mua
    # PARTIAL (buffer $200; dâu 4-event ROI ~19x), phần dứt khoát không nổi
    # redirect sang CARROT (không phải feed). Δ9: CHỈ khi nq>=2 (sau NE) —
    # trước khi mua đất phải giữ vốn (kain37 s100: straw-partial $300 d5h0
    # -> NE không bao giờ mua được -> farm kẹt 25 ô).
    ...
    if 4 <= day <= 13 and _nq_plan >= 2:
        try:
            ...
            if _sk > _seedable:
                ...
                _over_tot += _sk - _seedable
            _mk = crop_tiles.get("MELON", 0)
            _mseed = max(0, int((money - 300) // CROPS["MELON"]["seed"]))
            if _mk > _mseed:
                crop_tiles["MELON"] = _mseed
                _over_tot += _mk - _mseed
            if _over_tot > 0:
                _redir = min(_over_tot, 8)
                crop_tiles["CARROT"] = crop_tiles.get("CARROT", 0) + _redir
            # Δ1'' fill-carrot: ô trống còn lại -> carrot, cap tổng 14 ô
            # (carrot không phải feed — v6 không mua; giá crash hinge T=450)
            if remaining > 0:
                _c_now = crop_tiles.get("CARROT", 0)
                _c_room = max(0, 14 - _c_now)
                if _c_room > 0:
...
def _build_orders(me, shed, seeds, inventories, inv, prices, day, hour, plan,
    ...
    def _hold(it):
        h = HOLD.get(it, 0.90)
        # E1 WHEAT-HOLD (kain39): d6-16 chỉ bán wheat >= $33.75 (dưới gate
        # $38 của v6 — v6 vẫn mua nhưng trả đắt; dư dồn sang d17+ giá $40-49)
        if it == "WHEAT" and 6 <= day <= 16:
            h = max(h, 1.35)
        if it in glutted:
            return min(h, 0.40)
        if 22 <= day <= 27:
