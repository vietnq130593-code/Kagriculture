...
# với 3 đòn tinh chỉnh dành riêng cho đối đầu trực tiếp với kme3:
#   T1 CADENCE-RACE: R36 reservation horizon 5 (kme3 = 4) → mỗi lot bán trước
#      kme3 đúng 1 turn trên TOÀN bộ cadence d12-28 → giá cao hơn hệ thống.
#   T2 EARLY-R36: mở cửa sổ R36 từ đầu mùa (kme3 chỉ d12+) — chỉnh qua _V12_R36_LO.
#   T3 TERMINAL-PREDUMP: bán sạch shed 1 turn trước đối thủ ở cuối trận
#      (kme3 dump tại 718; v12 dump tại _V12_PREDUMP_STEP) — chỉnh qua hằng số.
# Baseline kiểm chứng: kme3 self-play s100 = HÒA TUYỆT ĐỐI $78.443/bên.
...
# ---------------------------------------------------------------- V12 config
_V12_HORIZON = 5        # T1: R36 reservation horizon (kme3 = 4)
_V12_R36_LO = 288       # T2: R36 window start (kme3 = 288 = d12)
_V12_PREDUMP_STEP = 717  # T3
...
def _r36_reserve(obs,action):
    ...
    if not _V12_R36_LO<=step<696:return action
...
def agent(observation, configuration=None):
    ...
    state['step'] = step
    # V12-T2: horizon mở cho toàn bộ cửa sổ R36 (khi _V12_R36_LO < 288),
    # ngoài cửa sổ giữ gốc 2 như kme3.
    _R37_HORIZONS[player] = _V12_HORIZON if (_V12_R36_LO <= step < 696) else 2
    ...
    if 288 <= step < 696:_R37_HORIZONS[player] = _V12_HORIZON
