...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money, tm=None):
    ...
    # KAIN-4: herd = v5's adaptive formula + shop floors, but DELIVERY-
    # disciplined: capital-capped early (no 15-pasture land grab while broke),
    # labor-safe total cap (13 — kain3's straw engine died at 17 animals),
    # faster buys (2/day to d14) + price-gated windows in _build_orders.
    goose_target = max(3, min(6, int(egg_room // 46)))
    ...
    try:
        ...
        pass
    if day <= 10:
        # capital-capped early buildout: never reserve pastures we cannot
        # stock (kain4-v1 reserved 15 at $223 and strangled its own straw)
        cow_target = min(cow_target, 1 + int(money // 900))
        sheep_target = min(sheep_target, 2 + int(money // 1500))
    try:
        _wdk = int((_STATE.get(("risk", day - 1)) or {}).get("water_crit_late", 0) or 0)
    except Exception:
        _wdk = 0
    if _wdk >= 3:
        # KAIN-7 (v5.2 F3): labor-starved yesterday -> shrink herd targets
        cow_target = max(2, cow_target - 2)
        sheep_target = max(3, sheep_target - 2)
        goose_target = max(3, goose_target - 2)
    ...
    if tot > 13:
        ...
        tot = cow_target + sheep_target + goose_target
        if tot > 13:
            sheep_target = max(3, sheep_target - (tot - 13))
    if day < 2:
        goose_target = 0
        cow_target = 0
        sheep_target = 0
    ...
    if 5 <= day <= 13:
        ...
        # KAIN-2: full 30-straw commitment from d5 (v4 ramped 14 until d8)
        want_straw = 30
        # KAIN-4e: port v5's minimax + scarcity escalation — v5's straw field
        # is 26-28 tiles on shared seeds while kain's room() (always minus
        # 0.85*opp_pipe) yields the channel and its $900 seed floor blocks
        # the broke days d5-11.
        try:
    ...
    if day <= 24:
        if day <= 4:
            ...
            # KAIN-4d: strawberry ~2x wheat $/action — shift tiles wheat->
            # straw ONLY when straw quota will actually fill them (s_room
            # gate passed); 4c cut wheat even when straw was room-blocked.
    ...
    # KAIN-7 (v5.6 Wave C): don't quota crops that cannot mature by d29
    try:
            ...
            if kind == "PLANT":
                ...
                if not watered:
                    ...
                    if in_win:
                        ...
                        # KAIN-7 (v5.2 fix): missed yesterday -> tier 0
                        # UNCONDITIONALLY. v4's elif let MAINT starve on the
                        # parity-ON day and the tile died at cu=2.
                        ...
                        if on_day:
                            _wd = 0
                            try:
                                _wd = int((_STATE.get(("risk", day - 1)) or {})
                                          .get("water_crit_late", 0) or 0)
                            except Exception:
                                _wd = 0
                            t_w = T_WATER_MAINT if _wd < 1 else T_SERVICE
...
def _build_orders(me, shed, seeds, inventories, inv, prices, day, hour, plan,
    ...
    if hour <= 8 and 1 <= day <= 22:
        ...
        # KAIN-4 delivery: price-gated wider windows + 2/day through d14
        ...
        for animal, target, w0, w1 in (("GOOSE", plan.get("goose_target", 0), 2,
            ...
            if (owned < target and w0 <= day <= w1
                ...
                money -= ad["cost"]
        # herd-first capital: only the luxury crop (MELON) defers to pending buys
        if 4 <= day <= 15:
            try:
            ...
            if have < n_tiles:
                ...
                if crop == "STRAWBERRY":
                    ...
                    try:
                        ...
                        # KAIN-6: low straw-seed floor ONLY when capital can
                        # survive the batch (seed 101 seat0: floor-200 while
                        # broke ate the whole treasury at d5-10 -> $33k game).
    ...
    # KAIN-4d: keep 4 days of feed in the shed — stop the sell-at-h0 /
    # buy-back-at-h3 churn that burned ~$25k/game vs v5's self-fed herd.
    wheat_reserve = min(animals * 4 + 6, shed.get("WHEAT", 0) or 0) if day < 26 and shed else 0
