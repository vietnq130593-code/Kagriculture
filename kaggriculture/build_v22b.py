#!/usr/bin/env python3
"""Assemble v22b.py — flat single-namespace build of the v19.4 chain + v22
MILK-BANKER (tuned) layer (host = the v20 chain's entry point).

    v20.py chain (v18 + v19 + v191 + v192 + v193 + v194, verified flat build)
    + v22b_layer (milk recovery banker)                   — host: agent (v20)
...
Run from the project root:  python3 build_v22b.py
"""
import os
...
v20_src = read("v20.py")
v22b_layer = read("v22b_layer.py")

v22b_layer = v22b_layer.replace(
    "_V22B_HOST = agent  # noqa: F821  (flat chain: v19.4's entry point)",
...
header = '''# v22 "MILK-BANKER (tuned)" — flat build of the v19.4 chain + v22 layer.
# Chain (identical logic to v20.py + v22b_layer.py, single namespace):
...
def agent(observation, configuration=None):
    return _V22B_ENTRY(observation, configuration)
...
out_path = os.path.join(ROOT, "v22b.py")
with open(out_path, "w", encoding="utf-8") as f:
    f.write(src)

size = os.path.getsize(out_path)
print(f"v22b.py written: {size:,} bytes")
