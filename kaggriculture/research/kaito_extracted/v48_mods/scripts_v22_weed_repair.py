...
def wrap_weed_repair(policy, base_actions, *, replay_steps=8, stop_on_match=False):
    ...
    def wrapped(obs):
        ...
        game = states[seat]
        # A sparse router may instantiate every child at import time but call
        # a continuation for the first time only after its public decision
        # point.  Treat that first late call as a clean game initialization;
        # otherwise ``game["active"]`` is missing and the whole agent can
        # silently fall back to PASS.
        if (
            step == 0
            or "active" not in game
