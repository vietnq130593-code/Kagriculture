# v26 'HERD-ADAPTIVE ANSWER GENERAL' (Task 100, 19-09) — v25.1 + 4 lớp port từ
# thomastschinkel 'The 2945 Farm v9/4' (nghiên cứu Task 99): (1) HERD-ADAPTIVE
# _Y_CFG nosheep/nogeese + window (6,11) — xem comment tại _Y_CFG; (2) RACEGATE
# glut-gate + horizon 40; (3) ORDERPRI2 exposure-reorder; (4) CAPHARV pre-cap
# harvest — toàn bộ race-conditional: TẮT khi máy race phát hiện cùng-chassis,
# giữ nguyên hành vi v25.1 (48W-0L vs ahmedv48). Chi tiết: phần v26 layer cuối file.
...
# Task 100 (v26) — HERD-ADAPTIVE: port rule herd đọc-shop của 2945-farm
# (thomastschinkel 'The 2945 Farm v9/4'): s110 shops = SMOOTHIE+ICE_CREAM×2+PET_CAFE
# +PIZZA → cầu MILK 24/ngày, WOOL chỉ còn 1/ngày (town center) vì YARN không mở:
# cừu = dead weight $500/con, 2945 chuyển 12 bò và MILK giữ $220-256 cả mùa.
# nosheep: SHEEP→COW khi không-YARN và ≥1 milk shop; nogeese: GOOSE→COW/SHEEP khi
# không-egg-shop (BAKERY/BRUNCH). Window (8,11)→(6,11) khi không-race (xem
# _y_controller): shop unlock cuối ngày 2/5/8/11 — ngày 6 đã thấy 2 shop milk.
_Y_CFG={'yarnsheep': True, 'yarngeese': True, 'days': (8, 11), 'v26v9': True}
...
def _y_target(kind,shops,cfg,prices=None):
    ...
    # Task 100 (v26b) — verdict COW của 2945-farm V9 herd, port CHÍNH XÁC các
    # gate V9_HERD_*: egg_shops==0 (V9_HERD_MAX_EGG_SHOPS_COW=0) AND YARN chưa
    # mở AND milk_shops>=3 (V9_HERD_MIN_MILK_SHOPS) AND MILK>=150
    # (V9_HERD_MIN_MILK) → SHEEP/GOOSE mua trong window (8,11) đổi thành COW
    # (profile "bò chuyên sâu" s110, +$8-10k/ghế theo mổ xẻ Task 99). KHÔNG
    # đánh cược YARN tương lai: mọi cấu hình shop khác giữ nguyên mix v25.1
    # (verdict chỉ là THÊM, không chặn các rule gốc bên dưới chạy tiếp).
    if cfg.get('v26v9') and prices is not None and kind in ('SHEEP','GOOSE'):
        try:
            milk_p = int(prices.get('MILK', 0) or 0)
        except Exception:
            milk_p = 0
        if (not egg) and (not yarn) and milk >= 3 and milk_p >= 150:
            return 'COW'
...
def _y_controller(obs,action,state,cfg):
    ...
    # Task 100 (v26): mở rộng window (8,11)→(6,11) chỉ khi KHÔNG race
    # cùng-chassis (đối thủ ngoài dòng như 2945-farm cần quyết định sớm từ
    # ngày 6 khi shop milk đã lộ); khi race giữ nguyên window v25.1 để bảo
    # toàn hành vi 48W-0L vs ahmedv48.
    _v26_win = cfg['days']  # Task 100 (v26b): giữ window (8,11) gốc v25.1
    if _v26_win[0]<=day<=_v26_win[1]:
        for o in market:
            if len(o)>=3 and o[0]=='BUY_ANIMAL' and o[1] in _Y_COST and 1<=int(o[2])<=cfg.get('maxq',2):
                to=_y_target(o[1],shops,cfg,obs['market']['prices'] if cfg.get('v26v9') else None)
...
# ================================================================== v25 layers
# v25 "THE LAST DAY GENERAL" (Task 95, design: kaggle-research/v25-blueprint.md).
# Base: ahmedberatozer's V48 "Clear the Queue" (kaggle.com/code/ahmedberatozer/
# kaggriculture-v48-clear-the-queue, Apache-2.0, byte-exact above — the 36/0
# held-out machine that took the meta from our v24). v25 layers the ONE
# mechanism family the public V43..V48 lineage does not have: our Task-92
# H2 garbage-price throttle (hoard-release), plus a slot-cleanup compact
# after it. In the exact-mirror matchup (identical parents) the outer layer
# decides: the throttle's mid-game hold/release won d15-24 by +$466..+$1,247
# in the Task-93/94 batteries even on the weaker v24 chassis.
#
# Task 96 (quiet-seed autopsy, s119/s106/s121 forensics) — the two-band
# drain-eta throttle: (1) FERTILIZER is never hoarded (R40: the town consumes
# every item EXCEPT fertilizer -> its price decays monotonically, strips only
# strand tail units); (2) true-bottom sells (p < 5) hoard unconditionally;
# (3) the $5..14 band hoards only with a drain-backed recovery path (a shop
# consumes the item AND pure-drain closes the overshoot to the release price
# within 6 days, before the endgame flush) — a bare WOOL-style strip there is
# the d16 cliff-delay (strip @ $5 -> tape re-sells @ $1 a turn later);
# (4) from day 27 the hoard flushes at any price (units otherwise die unsold).
# Battery t96_v25e_full vs ahmedv48: 44W-2L-2T (91.7%), mean +$215, CI95
# [$152, $283]; vs v24 10-0 (+$813); vs ahmedv46 10-0 (+$2,086); mirror ties.
#
# Task 97 (new-opponent response, alperen1 + tetsutani forensics) —
# sale-advance horizon race: alperen5252525 "First in Line — Stock Into
# Income" (19-09 pull, sha256 53dc224a…, byte-exact V48 + _ADV_LOOK=8)
# took the s118 money-flow apart: identical WOOL units sold but +$1,449
# more (earlier slots, pre-drop quotes), MILK +6u +$1,342 — the lookahead
# race in the shared tape. v25 (Task 96, look=3) fell to 14W-34L (29%).
# Round 1 parity _ADV_LOOK=8: 91.7% vs alperen1 but still 2W-8L vs
# tetsutani v65 "Demand-Preserving Fourteen-Turn Sale Timing" (public
# notebook, V48 + _ADV_LOOK=14 + bakery/yarn guards, sha256 95b02b9b…).
# Final: rebind _ADV_LOOK=14 (the same public one-line parameter family,
# Apache-2.0 lineage, mechanism already in the chassis; tetsutani's
# bakery/wool guards deliberately NOT ported — they handicap the agent
# in the shared-tape race, measured 8W-2L for guardless look-14 vs v65).
# v25g batteries: vs alperen1 48W-0L (100%), +$723, CI95 [$594, $857];
# vs ahmedv48 46W-2L (95.8%), +$494, CI95 [$382, $610]; vs tetsutani_v65
# 44W-4L (91.7%), +$370, CI95 [$275, $471]; vs v24 10-0 (+$785).
_V25T_ENTRY = _e335_agent
# --- Task 96 (quiet-seed autopsy, s119) ---
# FERTILIZER never recovers (R40: town consumes every item EXCEPT fertilizer,
# so its market inventory is non-decreasing -> price decays monotonically all
# season). Stripping a FERT sell only delays revenue and strands tail units
# (s119: 1 FERT died unsold at step 718). Never hoard it.
_V25T_NEVER_STRIP = frozenset({'FERTILIZER'})
# Endgame flush: from day 27 the hoard unwinds at ANY price - units still
# held past the tape's last scheduled sell otherwise die unsold.
_V25T_FLUSH_DAY = 27
_V25T_FLUSH_CHUNK = 12     # per-item per-turn dump cap (price discipline)
_V25T_GATE_P = 15          # strip only true-garbage prices
_V25T_RELEASE_P = 18       # release back into recovered prices
_V25T_HOURS = set(range(0, 21))   # h0-h20 only (h21+ = preguard territory)
_V25T_MAX_ORDER = 200      # REAPER-sized orders always pass
_V25T_HOARD_CAP = 24       # total units held across items
_V25T_SHED_INTERLOCK = 55  # no new strips when the shed is this full
_V25T_RELEASE_CHUNK = 12   # max units released per item per day
_V25T_START_DAY = 3
_V25T_END_DAY = 27
_V25T_CRASH_REF = 25    # item must have been worth >= this within 3 days
_V25T_HIST = 72         # crash-reference window (3 days)

# --- Task 96 drain-eta oracle ---
# s106/s119/s121 forensics: the throttle's hold only pays when the item's price
# can actually RECOVER, and recovery is drain-driven (the engine's town consume,
# kaggriculture.py _town_consume: every 4 steps each unlocked shop instance
# consumes 1 unit (x2 for single-product shops) of each of its products; every
# 24 steps the town center consumes 1 of every item but FERTILIZER).  WOOL with
# no YARN_STORE (drain 1/day) never recovered in ANY quiet seed while MILK
# (drain 7/day) recovered in 2-6 days in every crash seed.  Hold only when a
# shop backs the item AND the pure-drain recovery ETA fits before the flush.
_V25T_MIN_DRAIN = 6      # = one shop instance; town-center alone never suffices
_V25T_ETA_MAX = 6.0      # days; recovery must arrive before the endgame flush
_V25T_SHOP_ITEMS = {
...
_V25T_INV_TARGETS = {}   # memoized inventory level at the release price


def _v25t_drain_day(item, shops):
    """Units/day the town removes from the market for this item (engine-exact)."""
    d = 0 if item == 'FERTILIZER' else 1
    for sh in shops or []:
        d += 6 * _V25T_SHOP_ITEMS.get(sh, {}).get(item, 0)
    return d


def _v25t_inv_target(item):
    """Market inventory at which the quote reaches the release price (above-curve
    is monotone; bisection on the extracted engine market model)."""
    if item in _V25T_INV_TARGETS:
        return _V25T_INV_TARGETS[item]
    tgt = 10060
    try:
        params = {k: dict(v) for k, v in _R37_MARKET_PARAMS.items()}
        lo, hi = 10000, 12000
        while lo < hi:
            mid = (lo + hi) // 2
            if _r37_market_price(item, mid, params) <= _V25T_RELEASE_P:
                hi = mid
            else:
                lo = mid + 1
        tgt = lo
        ...
        pass
    _V25T_INV_TARGETS[item] = tgt
    return tgt


def _v25t_hold_ok(item, obs, day):
    """True when holding this item through the crash has a drain-backed recovery
    path: a shop consumes it, and pure drain closes the overshoot to the release
    price within ETA_MAX days, landing before the endgame flush."""
    try:
        shops = (obs.get('town') or {}).get('unlocked_shops') or []
        drain = _v25t_drain_day(item, shops)
        if drain < _V25T_MIN_DRAIN:
            return False
        inv = ((obs.get('market') or {}).get('inventory') or {}).get(item)
        if inv is None:
            return False
        overshoot = _v25t_int(inv, 10 ** 6) - _v25t_inv_target(item)
        if overshoot <= 0:
            return True
        eta = overshoot / float(drain)
        if eta > _V25T_ETA_MAX:
            return False
        if day + eta > _V25T_FLUSH_DAY - 1:
            return False
    except Exception:
        return False
    return True

_V25T_STATE = {"held": {}, "released_day": {}, "err_day": -1, "errors_today": 0, "hist": {}}
_V25_TELEMETRY = dict(v25_changed=0, v25_removed=0, v25_errors=0,
                      v25t_stripped=0, v25t_released=0, v25t_errors=0)


def _v25t_int(x, default=0):
    try:
        return int(x)
    except (TypeError, ValueError):
        return default
...
def _v25_projected(obs, action):
    """Shed per item AFTER this turn's unit phase. Engine-exact for the three
    shed-touching ops (kaggriculture.py L343-406): DROP deposits the whole
    carried inventory (capacity-capped); PICKUP takes from the shed; PLACE at
    a shed-corner tile deposits non-structure items."""
    farm = obs['farms'][int(obs['player'])]
    private = obs['private']
    shed = {k: int(v) for k, v in dict(private.get('shed') or {}).items()}
    invs = [dict(v) if isinstance(v, dict) else {}
            for v in (private.get('inventories') or [])]
    positions = [list(farm['farmer'])] + [list(h) for h in (farm.get('hands') or [])]
    ...
    for i, cmd in enumerate(cmds):
        if i >= len(positions) or not isinstance(cmd, list) or not cmd:
            continue
        x, y = positions[i]
        if not (x in (c - 1, c) and y in (c - 1, c)):
            continue
        ...
        if op == 'DROP':
            for item, q in list(inv.items()):
                ...
                if take > 0:
                    shed[item] = shed.get(item, 0) + take
                inv[item] = 0
        elif op == 'PICKUP' and len(cmd) >= 2 and isinstance(cmd[1], str):
            item = cmd[1]
            try:
                q = int(cmd[2]) if len(cmd) >= 3 else 1
            except (TypeError, ValueError):
                continue
            q = max(0, min(q, shed.get(item, 0)))
            shed[item] = shed.get(item, 0) - q
            inv[item] = inv.get(item, 0) + q
        elif op == 'PLACE' and len(cmd) >= 2 and isinstance(cmd[1], str):
            item = cmd[1]
            try:
                q = int(cmd[2]) if len(cmd) >= 3 else 1
            except (TypeError, ValueError):
                continue
            q = max(0, min(q, inv.get(item, 0)))
            room = max(0, cap - sum(shed.values()))
            q = min(q, room)
            if q > 0:
                shed[item] = shed.get(item, 0) + q
...
_V25_ITEMS = {'CARROT', 'TOMATO', 'STRAWBERRY', 'MELON', 'EGG', 'MILK', 'WOOL'}


def _v25_compact(obs, action):
    market = action.get('market') or []
    if len(market) < 2:
        return action
    norm = [list(o) if isinstance(o, (list, tuple)) else o for o in market]
    all_sell = all(isinstance(o, (list, tuple)) and len(o) >= 3 and o[0] == 'SELL'
                   and isinstance(o[1], str) for o in market)
    remaining = dict(_v25_projected(obs, action))
    if not all_sell:
        segments = []
        i = 0
        while i < len(market):
            ...
            if (isinstance(o, (list, tuple)) and len(o) >= 3 and o[0] == 'SELL'
                    and o[1] in _V25_ITEMS):
                j = i + 1
                while (j < len(market) and isinstance(market[j], (list, tuple))
                        and len(market[j]) >= 3 and market[j][0] == 'SELL'
                        and market[j][1] in _V25_ITEMS):
                    j += 1
                if j - i >= 2:
                    segments.append((i, j))
                i = j
        ...
        for start, end in segments:
            ...
            for o in market[start:end]:
                ...
                try:
                    ...
                    pass
            # Task 98 (v25.1) FIX-A slot hygiene (v251e: ENDGAME-ONLY window —
            # h21/h22 preguard + day>=27 flush): within a merged SELL run, list
            # the biggest-revenue row first (qty * price, stable sort). The
            # engine interleaves the two players' market lists slot by slot, so
            # a junk-priced row parked before a valuable one queues our whole
            # dump behind the rival's same-item dump (s100 autopsy: MILK $3 at
            # slot 1 pushed STRAW 17u x$35 behind the rival's STRAW dump ->
            # -$530 in one turn). Mid-game reordering cascades through the
            # advance layer's bookkeeping (s107 autopsy: identical STRAW/WOOL
            # unit flows, butterfly price paths from d20 -> -592 on a thin
            # seed) — so v251e fires the reorder ONLY in the endgame windows
            # where the big shared dumps actually happen.
            _v251e_gate = (obs.get('step', -1) % 24 in (21, 22)) or (obs.get('step', -1) // 24 >= 27)
            try:
                _px = ((obs.get('market') or {}).get('prices')) or {}
                _order = sorted(order, key=lambda it: -_v25t_int(_px.get(it), 0) * _v25t_int(quantities.get(it), 0)) if _v251e_gate else list(order)
            except Exception:
                _order = list(order)
            kept = []
            for p in _order:
                avail = max(0, int(remaining.get(p, 0)))
                q = min(quantities[p], avail)
                if q:
                    kept.append(['SELL', p, q])
                    remaining[p] = avail - q
            replacement = kept + [[] for _ in range(end - start - len(kept))]
            if replacement != [list(o) for o in market[start:end]]:
                removed += end - start - len(kept)
                new[start:end] = replacement
        if new == norm:
            return action
        _V25_TELEMETRY['v25_changed'] += 1
        _V25_TELEMETRY['v25_removed'] += removed
        return dict(action, market=new)
    effective = []
    for o in market:
        item = o[1]
        try:
            ...
            q = 0
        avail = max(0, int(remaining.get(item, 0)))
        q = min(max(0, q), avail)
        remaining[item] = avail - q
        effective.append(['SELL', item, q] if q else [])
    new = [list(o) for o in effective]
    i = 0
    removed = 0
    while i < len(effective):
        if effective[i] and effective[i][1] not in _V25_ITEMS:
            i += 1
            continue
        j = i + 1
        while j < len(effective) and (not effective[j] or effective[j][1] in _V25_ITEMS):
            j += 1
        order = []
        qty = {}
        for o in effective[i:j]:
            ...
            if o[1] not in qty:
                ...
                qty[o[1]] = 0
            qty[o[1]] += o[2]
        # Task 98 (v25.1) FIX-A slot hygiene (all-sell path, v251e endgame-only
        # gate): revenue-desc ordering of the merged rows, same rationale and
        # same window as the segment path.
        _v251e_gate = (obs.get('step', -1) % 24 in (21, 22)) or (obs.get('step', -1) // 24 >= 27)
        try:
            _px = ((obs.get('market') or {}).get('prices')) or {}
            _order = sorted(order, key=lambda it: -_v25t_int(_px.get(it), 0) * _v25t_int(qty.get(it), 0)) if _v251e_gate else list(order)
        except Exception:
            _order = list(order)
        kept = [['SELL', item, qty[item]] for item in _order]
        new[i:j] = kept + [[] for _ in range(j - i - len(kept))]
        removed += j - i - len(kept)
        i = j
    if new == norm:
        return action
    _V25_TELEMETRY['v25_changed'] += 1
...
def _v25t_throttle(obs, action):
    """Task-92 H2 garbage-price throttle, ported self-contained from our v24:
    strip SELLs priced below the gate into a bounded hoard, release them
    chunked (1 item/day) once the price recovers past the release level."""
    step = _v25t_int(obs.get('step'), -1)
    day = step // 24
    if day < _V25T_START_DAY:
        return action
    flush = day >= _V25T_FLUSH_DAY
    hour = step % 24
    if hour not in _V25T_HOURS and not flush:
        return action
    if day >= _V25T_END_DAY and not flush:
        return action
    if _V25T_STATE.get('err_day') != day:
        _V25T_STATE['err_day'] = day
        _V25T_STATE['errors_today'] = 0
    if _V25T_STATE['errors_today'] >= 3:
        return action
    market_orders = action.get('market')
    if not isinstance(market_orders, list) or not market_orders:
        return action
    prices = ((obs.get('market') or {}).get('prices')) or {}
    # price history: strip only CRASHES (recently-worthwhile items that fell
    # below the gate), not chronic garbage — the Task-95 forensics showed the
    # shield pays +$300..700 through crash recoveries (s103 MILK 72->24->1)
    # and costs -$8..16 of friction on permanently-dead items (s101 WOOL 1-5).
    hist = _V25T_STATE.setdefault('hist', {})
    for it, pv in prices.items():
        h = hist.setdefault(it, [])
        try:
            h.append(int(pv))
        except (TypeError, ValueError):
            continue
        if len(h) > _V25T_HIST:
            del h[:len(h) - _V25T_HIST]
    def _v25t_recently_worth(item):
        h = hist.get(item) or []
        return bool(h) and max(h) >= _V25T_CRASH_REF
    shed = ((obs.get('private') or {}).get('shed')) or {}
    shed_total = 0
    try:
        shed_total = sum(_v25t_int(v) for v in shed.values())
    except Exception:
        shed_total = 0
    held = _V25T_STATE['held']
    held_total = sum(held.values())
    strip_ok = (shed_total + held_total) <= _V25T_SHED_INTERLOCK
    out = []
    stripped = False
    for o in market_orders:
        if (strip_ok and isinstance(o, (list, tuple)) and len(o) >= 3
                and o[0] == 'SELL' and isinstance(o[2], (int, float))
                and 0 < _v25t_int(o[2], 0) <= _V25T_MAX_ORDER):
            item = o[1]
            p = _v25t_int(prices.get(item), 10 ** 9)
            # two-band gate (Task 96): true-bottom sells (p < 5) hoard
            # unconditionally - the v25c battery showed this band is +EV even
            # in shop-less seeds (micro-wins +$2..6/seat); the $5..14 band
            # hoards only with a drain-backed recovery path (the v25 crash
            # wins) - a bare WOOL-style strip there is the d16 cliff-delay.
            if (item not in _V25T_NEVER_STRIP and _v25t_recently_worth(item)
                    and (p < 5
                         or (p < _V25T_GATE_P and _v25t_hold_ok(item, obs, day)))):
                q = _v25t_int(o[2], 0)
                room = _V25T_HOARD_CAP - held_total
                if room > 0:
                    held[item] = held.get(item, 0) + q
                    held_total += q
                    stripped = True
                    _V25_TELEMETRY['v25t_stripped'] += q
                    if held_total >= _V25T_HOARD_CAP:
                        strip_ok = False
                    continue
        out.append(o)
    released_any = False
    for item in list(held.keys()):
        phys = _v25t_int(shed.get(item), 0)
        if 0 <= phys < held.get(item, 0):
            held[item] = phys
        if held.get(item, 0) <= 0:
            if item in held and held[item] == 0:
                del held[item]
            continue
        p = _v25t_int(prices.get(item), 0)
        if p < _V25T_RELEASE_P and not flush:
            continue
        if _V25T_STATE['released_day'].get(item) == day and not flush:
            continue
        shed_units = _v25t_int(shed.get(item), 0)
        if shed_units <= 0:
            continue
        if flush:
            k = min(held[item], shed_units, _V25T_FLUSH_CHUNK)
        else:
            k = max(4, min(_V25T_RELEASE_CHUNK, p // 4))
            k = min(held[item], shed_units, k)
        if k <= 0:
            continue
        if len(out) >= 10:
            break
        ...
        _V25T_STATE['released_day'][item] = day
        released_any = True
        _V25_TELEMETRY['v25t_released'] += k
        if held.get(item, 0) <= 0:
            del held[item]
...
def agent(observation, configuration=None):
    action = _V25T_ENTRY(observation, configuration)
    try:
        if not isinstance(action, dict):
            return action
        if int(observation.get('step', -1)) == 0:
            for k in ('v25_changed', 'v25_removed', 'v25_errors',
                      'v25t_stripped', 'v25t_released', 'v25t_errors'):
                _V25_TELEMETRY[k] = 0
            _V25T_STATE.update(held={}, released_day={}, err_day=-1, errors_today=0, hist={})
        try:
            action = _v25t_throttle(observation, action)
        except Exception:
            _V25_TELEMETRY['v25t_errors'] += 1
            _V25T_STATE['errors_today'] = (_V25T_STATE.get('errors_today', 0) or 0) + 1
        market = action.get('market')
        if not isinstance(market, list) or len(market) < 2:
            return action
        return _v25_compact(observation, action)
    except Exception:
        _V25_TELEMETRY['v25_errors'] += 1
        return action


agent.telemetry = _V25_TELEMETRY


def _arena_diag(obs):
    try:
        return {'v25': dict(_V25_TELEMETRY),
...
# Task 97 (19-09) experiment: match alperen1's public parameter change
# ("First in Line — Stock Into Income", kaggle.com/code/alperen5252525/
# kaggriculture-first-in-line-stock-into-income): override EXP293
# sale-advance lookahead 3 -> 8 (module-level rebind, same mechanism
# alperen5252525 applies on top of the credited V48 core).
_ADV_LOOK=8


# Task 97 variant g: horizon parity with tetsutani v65 ("Demand-Preserving
# Fourteen-Turn Sale Timing", public notebook, Apache-2.0 lineage) — rebind
# EXP293 sale-advance lookahead 8 -> 14 (no guards yet).
_ADV_LOOK=14


# ================================================================== v25.1 layer
# v25.1 "DEMAND-PRESERVING RACE GENERAL" (Task 98) — port nghiên cứu từ
# tetsutani v65 "Demand-Preserving Fourteen-Turn Sale Timing" (public Kaggle
# notebook, Apache-2.0; main.py sha256 95b02b9b…; V48 + _ADV_LOOK=14 + 2 guards)
# RÚT RA SAU NGHIÊN CỨU TASK 98:
#   (1) Notebook v65 = V48-chassis + 2 guard "demand-preserving" cho lớp EXP293
#       sale-advance: (a) BAKERY guard — thấy >=2 BAKERY đang mở thì trả về
#       timing 3-turn của parent (cầu EGG+WHEAT mạnh, kéo sâu 14 turn là bán
#       vào đúng lúc giá đang hồi); (b) WOOL/YARN guard — offset 5..14 không
#       kéo WOOL bán sớm khi YARN_STORE đang mở (WOOL premium $240, shape
#       log/sq-3.2: dồn sớm tự đâm giá, tape bán phần còn lại vào giá sập).
#   (2) Mô phỏng shop-draw (R38: random.Random((seed*1_000_003)^day), unlock
#       d3/d6/…/d24, cap 8): YARN mở ở 9/11 seed chuẩn (từ d3!), BAKERY>=2 chỉ
#       ở s100 (d24) + s101 (d21) — guard WOOL là code SỐNG trong meta này,
#       guard bakery hiếm nhưng có thật.
#   (3) Đo thực nghiệm Task 97: guards VÔ ĐIỀU KIỆN = tự giải giáp trong
#       shared-tape race (v65 thua v25g 4W-44L). Đối tượng test của user
#       (alperen1/ahmedv48/v25) đều cùng chassis = race.
#   => Port dạng RACE-CONDITIONAL: guards tắt khi máy EXP283/288 phát hiện
#      race (mirror step-1 / clone-lock / escalation = horizon>=24) — giữ
#      nguyên đỉnh look=14 đã chứng minh; guards BẬT khi không race (đối thủ
#      ngoài dòng ahmed) — đúng tri thức demand-preserving của tetsutani.
#   (4) FIX-A slot hygiene (xửa lỗi s100, 2 trận thua duy nhất của v25g vs
#       ahmedv48): _v25_compact giờ sắp dòng SELL ghép theo doanh thu giảm
#       dần — xem comment FIX-A trong _v25_compact.
_V251_REPORT = dict(v251_bakery_turns=0, v251_wool_skips=0, v251_guard_errors=0,
                    v251_race_turns=0, v251_look3_turns=0)


def _v251_racing(player):
    """True khi đang đua cùng-tape: mirror gate step-1 (cash bằng nhau sau
    turn 1 = cùng chassis), escalation (đối thủ pre-empt tape của ta) hoặc
    clone-lock hiện thời (vị trí trùng + similarity >= 0.95)."""
    try:
        st = _RACE_STATE.get(player)
        if not st:
            return False
        return (st.get('horizon', 0) > 0
                or st.get('level', 0) >= max(_RACE_HORIZON_MIRROR, _RACE_HORIZON_ESCALATED))
    except Exception:
        return False


# Guarded rebuild của EXP293 _adv_apply — bản sao TRUNG THỰC của hàm gốc
# (xử lý y hệt khi guards tắt) + 2 guard demand-preserving của tetsutani v65,
# chỉ kích hoạt khi KHÔNG race. Rebind global `_adv_apply`: wrapper `agent`
# của lớp advance resolve tên này lúc gọi nên bản mới thắng.
_V251_ORIG_ADV = _adv_apply
...
def _adv_apply(obs, action):
    ...
    native = _IMPL.chassis.players[player]; debts = native['sell_state'].setdefault('r36_debts', {})
    plan = []; first = None
    # --- Task 98 (v25.1): tetsutani v65 guards, race-conditional ---
    try:
        shops = list(obs.get('town', {}).get('unlocked_shops', []) or [])
    except Exception:
        shops = []
        _V251_REPORT['v251_guard_errors'] += 1
    racing = _v251_racing(player)
    if racing:
        _V251_REPORT['v251_race_turns'] += 1
    adv_look = _ADV_LOOK
    if (not racing) and shops.count('BAKERY') >= 2:
        # Demand-preserving extension (tetsutani v65, nguyên văn cơ chế):
        # >=2 BAKERY đang mở -> giữ timing 3-turn của parent.
        adv_look = 3
    ...
    for off in range(1, adv_look + 1):
        ...
        for o in _adv_future(player, t):
            ...
            if first is None:
                first = o
            if (not racing) and off >= 5 and o[0] == 'SELL' and o[1] == 'WOOL' and 'YARN_STORE' in shops:
                # Demand-preserving extension (tetsutani v65, nguyên văn cơ chế):
                # để WOOL offset 5..14 nằm yên khi YARN_STORE đang mở.
            ...
            if o[0] == 'SELL' and o[1] in _ADV_ITEMS:
                try:
                    ...
                    q = 0
                if _ADV_SUBTRACT_DEBTS:
                    q -= debts.get(t, {}).get(o[1], 0)   # already reserved by the sale-reservation layer
                if q > 0:
                    plan.append((t, o[1], q))
    protected = first[1] if _ADV_PROTECT and first is not None and first[0] == 'SELL' else None
    plan = [(t, item, q) for t, item, q in plan if item != protected]
...
def _arena_diag(obs):
    """Task 98: merge v25 telemetry + v25.1 guard telemetry for the runner."""
    try:
        out = {'v25': dict(_V25_TELEMETRY), 'v251': dict(_V251_REPORT),
               'held': dict(_V25T_STATE.get('held') or {}),
               'released_day': dict(_V25T_STATE.get('released_day') or {})}
        return out
    except Exception:
...
# Production-loader hardening (kaggle server rule — bài học v20.1: server dùng
# get_last_callable() = [v for v in env.values() if callable(v)][-1], chọn
# callable có key chèn CUỐI vào globals; file này còn def _v251_racing/
# _adv_apply/_arena_diag SAU `agent` cuối cùng nên server sẽ gọi nhầm hàm
# helper. Fix pattern v65: pop + re-def `agent` ở dòng cuối file.)
_v251_policy = agent
globals().pop('agent', None)
def agent(observation, configuration=None):
    return _v251_policy(observation, configuration)
...
def _v26_capharv(obs, action, st):
    ...
    try:
        day = step // 24
        seat = int(obs['player'])
        # Task 100 (v26): race-conditional — tape cùng-chassis không bao giờ
        # tràn (audit Task 98 CLEAN), cướp thu sớm chỉ phá chuỗi care/fertilizer
        # (đo s100: −$203). Chỉ bật khi không race.
        if _v251_racing(seat):
            return action
        farm = obs['farms'][seat]
        private = obs['private']
        shed = private['shed']
