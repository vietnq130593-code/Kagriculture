...
Run from the project root:  python3 build_v22c.py
"""
import os
...
v20_src = read("v20.py")
v22c_layer = read("v22c_layer.py")

v22c_layer = v22c_layer.replace(
    "_V22C_HOST = agent  # noqa: F821  (flat chain: v19.4's entry point)",
...
# Chain (identical logic to v20.py + v22c_layer.py, single namespace):
...
def agent(observation, configuration=None):
    return _V22C_ENTRY(observation, configuration)
...
out_path = os.path.join(ROOT, "v22c.py")
with open(out_path, "w", encoding="utf-8") as f:
    f.write(src)

size = os.path.getsize(out_path)
print(f"v22c.py written: {size:,} bytes")
