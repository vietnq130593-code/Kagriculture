...
#
# GAP (sync91): dâu bán 41,6% (gap −$15k) · wheat máy 20% (−$10k) · carrot
# 10,5% · AD d0-10 68% (45,6 vs 67,5) · P2 33%.
#
# KHO-AUTOPSY (kho10.py — 4 seat top-3, 2 replay Kaggle gốc) — user hỏi số
# lượng kho + vị trí:
#   • 1 shed cố định/board (engine) — "nhà kho" = COOP/PASTURE, MỖI thú 1 ô
#   • Top-3 BUILD 4-6 kho NGAY d0 quanh shed (d̄shed 1,0-2,5, 1-4 cụm liền,
#     adj 0,87-1,0), đặt 4-6 thú ngay; kho trống chỉ 0-1 (không pre-build)
#   • Đàn 6-8 lúc d5, 13-15 lúc d10 (v9: 4 con d10 — burst muộn d11-16)
#   • v9 đã đạt: BFS-khối ôm shed (ncomp 1, d̄shed 0,6-1,7)
#
# V10 MATRIX (150 game engine thật, bat10/bat10b/bat10b2/bat10e_ext):
#   v10.0 (S1-S6 đủ): vs v6 48/50 TB $73,7k — melon 80→49u, wheat sell
#     KHÔNG tăng (FERT không land) → hụt $1,5k so v9.1
#   v10b (v9.1-opening + S1/S2/S3): 16/20 $70,3k — TỆ HƠN; AD 36,7 (!)
#   v10d (v10.0-opening + bỏ S1-late): 18/20 $74,1k — vẫn hụt
#   v10e (chỉ S6-opening + S5 + struct5): 19/20 $76,3k + thắng v9 6/10 ✅
#
# LUẬT ÂM V10 (quan trọng — ghi cho vòng sau):
#   A1. DÂU-FLAT cuối mùa (24/20 quota d14-26) ĂN VỐN HẠT + lao động tưới
#       event → melon wave-2 chết đứng (0,9 ô vs 3,1) = −$4,3k. Kernel
#       70 op/ngày KHÔNG đủ gánh 24 dâu + 20 wheat + wave-2 melon cùng lúc
#       (top-3 có 92-123 op/ngày). Thang tiered của v9.1 là tối ưu cho
#       kernel hiện tại.
#   A2. WHEAT-20-FLAT + FERT tier-2 + nước window tier-2 KHÔNG tự chuyển
#       thành sản lượng: FERT pool bị kênh dâu/melon/carrot giữ (sell 160u
#       không giảm), sell wheat 117u ≈ 114u. Trước khi nạp supply phải có
#       FERT-flow + window-execution thật (v11: vấn đề KERNEL ops, không
#       phải quota).
#   A3. Opening 2 bò + melon-12 (v10.0/v10d/v10e) NET DƯƠNG: AD 45,8
#       (+5,1), milk +$2,2k, đổi lại melon −$3k — thắng 19/20 giữ v9.1-level
#       + ĐẦU-TRỰC v9 6/10.
#   A4. buy_per_day 3 (d3-10) KHÔNG tự tăng AD nếu vốn đã kẹt hạt
#       (v10b AD 36,7 < v9.1 40,7) — AD chỉ tăng khi opening nhả vốn thú
#       (v10.0/v10e: 2 bò d0).
#
# V10e THỰC THI (file này): S6-opening (2 bò + 1 cừu + 1 ngỗng $1.600 +
# melon 12) + buy_per_day 3 d3-10 + struct-reserve cap 5 (kho d0 burst 4)
# + carrot cap 10/8 (dâu khỏe) + REVERT S1/S2/S3/S4 về đúng v9.1.
#
# KẾT QUẢ V10e: vs v6 48/50 (96%) TB $76,3k max $97,3k | thắng v9 đầu-trực
# 6/10 ($73,6k vs $72,1k) | vs v8 7/10 $74,4k | vs v7 10/10 $79,2k max
# $99,9k. SYNC 26 chỉ số: 75,3% (v9.1: 74,9%). Final $79,8k TB (77,9% mức
# top-3, 85,5% mức min — CHƯA vào top-3). Kho: d0 4 kho + đàn 4, empty
# 0,6-0,9, khối liền d̄shed 0,8 — ĐỒNG BỘ top-3.
#
...
def _struct_reserve(need, unit_cost, money):
    ...
    # V10-S6: cap 3 → 5 (kho10.py: top-3 BUILD 4-6 kho NGAY d0 quanh shed —
    # 4 kho đón 4 thú starter; builds free, chỉ tốn unit đứng tại chỗ).
    if need <= 0:
        return 0
    return min(need, 5 if money >= 300 else (2 if money >= 100 else 1))
...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money, tm=None):
    ...
    _herd_cap = 15  # R168: đàn 13 = top-3 parity (đàn 19 + máy chết = mua 610u wheat $31k net -$8k; top-3: đàn 13 + máy 20 = net +$10k + lao động dư tưới)
    ...
    # d15 bằng tiền melon).
    if day == 0:
        # v10d (giữ opening v10.0 — đo 2 battery: 2 bò + melon-12 cho
        # AD 45,8 (+5 vs v9.1) và milk +$2,2k, NET dương so với 1 bò +
        # melon-14 của v10b 16/20 — động vật trả nhiều hơn melon chiếm).
        cow_target = min(cow_target, 2 if money >= 2000 else (1 if money >= 1600 else 0))
    ...
    if 5 <= day <= 13:
        ...
        # S1 DÂU FLAT (v10): top-3 đứng 24,8 phẳng CẢ MÙA (bán 254u); v9.1
        # hạ 20/16-14/16-12 nửa sau mùa làm kênh chết sớm $15k. Kernel v9
        # walk/op 1,3 + PASS 920 unit-h → đủ lao động giữ 24-26 đứng suốt.
        # 26 (cũ 28) nhả 2 ô cho máy wheat 20 + carrot (tổng đất = đúng
        # cơ cấu top-3: 24,8 dâu + 20,2 wheat + 5,3 carrot + 4,5 melon).
        want_straw = 28
        ...
        # v10d: REVERT S1-late về thang v9.1 (20) — đo bat10+bat10b2: dâu
        # FLAT 24/20 cuối mùa ăn vốn hạt + lao động tưới event → melon wave-2
        # chết đứng 0.9 ô (v9.1: 3.1) = −$4.3k; kernel 70 op/ngày chưa đủ
        # gánh 24 dâu + 20 wheat + wave-2 melon cùng lúc (top-3 có 92-123).
    ...
    if day <= 26:
        if day <= 4:
            ...
            # Trụ 5-fix (R159) + R169/R170 + V10-S2: WHEAT 20 FLAT d5-26 —
            # máy là NHÀ MÁY THỨC ĂN + resale (top-3 đứng 20,2 ô cả mùa, mua
            # 248u nuôi đàn, bán 574u net +$10k; v9.1 cũ max(16, đàn+3) →
            # đàn 13 chỉ đứng đích 16 = máy non 20% sync).
    ...
    if remaining > 0 and day <= 21:
        ...
        if _straw_standing < 12:
            ...
            # V10-S5 CARROT CHUNG SỐNG: cũ 8/6 — dâu FLAT 26 vẫn để dư ô cho
            # carrot 10 (d≤19) / 8 (d20+): cơ cấu đất = 26 dâu + 20 wheat +
            # 10 carrot + 4 melon = 60/60 plantable; carrot bán 135u@$45 của
            # top-3 là kênh đứng rẻ #1 (hạt $20, 3u/4 ngày).
            _car_cap = 10 if day <= 19 else 8
        if money < 400:
            _car_cap = 0
        else:
            try:
                ...
                if not watered:
                    ...
                    if cd["ongoing"]:
                        ...
                        if cu >= 1 and crop == "MELON" and stats.get("melon_crit", 0) < 4:
                            ...
                            # V10b: REVERT S4 (window-water tier-2) — đo bat10:
                            # phase-2 crowding giết melon wave-2 (49u vs 80u của
                            # v9.1) mà wheat sell KHÔNG tăng (117u ≈ 114u) —
                            # nước window phải chờ FERT thật sự land trước.
    ...
    if hour <= 17 and seeds is not None:
        ...
        for crop in _seed_order:
            ...
            if not n_tiles and crop != "WHEAT":
                continue
            # R170 + V10-S2 (20-flat): seed-buy wheat nhìn standing SỐNG
            # hiện tại: floor 20 − live (đúng 3 layer plan/refill/seed-buy).
