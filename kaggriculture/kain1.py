...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money, tm=None):
    ...
    # KAIN-1c "DEEP DAIRY BLITZ": committed dairy, capital-sequenced.
    # v5.6 Wave-B floor + room logic under-defends deep milk markets; 12 cows
    # (reachable within capital curve) + COOP-mask flows tax v5 into yielding.
    goose_target, cow_target, sheep_target = 4, 12, 2
    if day < 2:
        goose_target = 0
        cow_target = 0
        sheep_target = 0
...
def _build_orders(me, shed, seeds, inventories, inv, prices, day, hour, plan,
    ...
    if hour <= 8 and 1 <= day <= 22:
        ...
        for animal, target, w0, w1 in (("GOOSE", plan.get("goose_target", 0), 2, 14),
                                       ...
                                       # KAIN-1c: cow window extended 16 -> 19 (late cows
                                       # still pay back at $300+ milk; capital arrives d15+)
                                       ("COW", plan.get("cow_target", 0), 5, 19)):
            ...
            if (owned < target and w0 <= day <= w1
                    ...
                    # KAIN-1b: relax wheat gate 1.3 -> 1.0 (feed engine scales to 30 tiles)
                    and (wt_supply >= (animals_total + 1) * 1.0 or animals_total == 0)):
    ...
    if nq < 4 and bought.get("LAND", 0) < 1:
        ...
        if nq == 3:
            # KAIN-1c: skip SE ($4k) — capital reserved for the dairy herd
            last_day = -1
            gate = 1.1
    ...
    if animals > 0 or plan_animals > 0:
        ...
        if shed_wheat < wheat_want and money >= 400:
            ...
            n = min(need, afford)
            acute = shed_wheat < 6
            # KAIN-1b: dairy pays feed premium (milk $200+ covers $48 wheat)
            if n >= 1 and (pw <= 48 or (acute and pw <= 62)):
