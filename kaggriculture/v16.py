...
_H8_STATE={'prev':{0:None,1:None},'on':{0:False,1:False},'active':False}
...
def _h8_sync(obs):
    try:
        ...
        if opp is not None:
            if prev is not None and step in (217, 218, 219) and prev-float(opp)>=100 and not _H8_STATE['on'][seat]:
