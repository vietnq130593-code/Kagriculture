# v16 — ARI CLASS Mk-III (Task 75, 2026-09-13). Kaggle-research edition.
# Base: v15 (ARI CLASS Mk-II, Task 73) = v14 + R90 shed-animal watchdog.
# + NEW v16 layers (Task 75, from the Task-74 Kaggle public-meta research —
#   493 notebooks, 34 analyzed code-level; see research/03_BREAKTHROUGH_PLAN.md):
#   [H1 TOWN-DEMAND GATE]  _sell_lead/_front_run now skip pull-forward for an
#     item on steps where the town consumes it (centre dawn drain step%24==0 +
#     open-shop ticks step%4==0 via _SHOP_PRODUCTS). Sources: salemali7 / boatlee
#     V16-RC5 / andrew munib_FR (3 independent notebooks). The crude step%4==0
#     blanket gate in _sell_lead was replaced by this per-item check.
#   [H2 DEAD_STOCK + TERMINAL_LIQUIDATION ON]  sells route-orphaned stock
#     (price>1) and liquidates the whole projected shed at step 718. Measured
#     near-inert on our clean schedule (same-dollar A/B) — kept as endgame
#     insurance. Sources: tetsutani + thomast (+19/-0 claim) + fieldbook.
#   [H3 R88 HORIZON-FEED]  ported verbatim from KaggressurE V39 (guruprasaathas
#     EXP219): feed-bonus cost counts pending_care_bonus only on production
#     dawns (banked otherwise) and care=0 when the next payable dawn is past
#     day 29 — no useless endgame feeding.
# Battery (Task 75, seeds 330-334 x 2 seats, 90 games): 86W-0L-4T.
#   vs v15 8/10 +$331 | v14 8/10 +$331 | v13 10/10 +$5,274 | kme3 10/10 +$5,290
#   | kme3v10 10/10 +$4,965 | aurax 10/10 +$4,972 | kme3v39 10/10 +$4,510
#   | kawashigi 10/10 +$28,070 | indark_e776 10/10 +$14,299.
# Construction: v15.py + H1 helper block + _View.shops + 2 gate insertions
#   + _SETTINGS dead_stock/terminal_liquidation flip + _r88_feed_bonus_cost port
#   (exact recipe in worklog.md Task 75).
# Base: v14 (ARI CLASS, Task 69) = kme3v10 + 6 v13 tuning layers + front_run/H8/
#   room_guard/clamp_sells (see v14.py header for the full stack).
# + NEW v15 layers (from the Task 72 Kaggle-loss autopsy):
#   [R90 SHED-ANIMAL WATCHDOG]  root cause of the -$5,973 loss to Dmitriy Ulybin
#     (seed 661817856): a COW bought into a full pasture -> worker PICKUP ->
#     PLACE failed -> end-of-day dump returned it to the shed -> the planner
#     never retried; the cow sat in the shed 26 days while pastures stood empty.
#     R90 rescues any animal that has been orphaned in the shed >= its grace
#     period (COW/GOOSE 24 steps, SHEEP 48) onto the nearest empty matching
#     structure tile, using the farmer (free at h0, spawns on a shed-access
#     tile). Also reproduced locally: seed 300 seat 1 (cow stuck t268->t718).
# All measurement tools: bench/battery.py (2-job two-sided battery),
# bench/gap_waterfall.py (per-item/day gap attribution), bench/force_route.py
# (route-variant generator + default-route probe).
...
# v14-M (Task 69, ARI plan H1): v13 tuning constants applied on the kme3v10 base.
_V12_HORIZON = 8
_V12_R36_LO = 144
_V12_R36_HI = 712
_V12_PREDUMP_STEP = -1
...
# ---- H1 (Task 75): town-demand gate for pull-forward layers -----------------
# salemali7 / boatlee V16-RC5 / andrew munib_FR converge: only front-run an item
# on a step where the town does NOT consume it (selling into a demand tick
# forfeits the post-drain price bump; without demand, selling a step early only
# gains queue priority over the next-step dumpers).
_SHOP_PRODUCTS = {
    "BAKERY": ("EGG", "WHEAT"),
...
def _town_demand_now(shops, item, step):
    """Units of ``item`` the town consumes this step, from public info only.
    Town centre drains 1 unit per non-FERT product at each dawn (step % 24 == 0);
    each unlocked shop drains its products every 4th step (x2 when the shop
    sells a single product). 0 when unknown/empty shops -> gate inert."""
...
# ---- H3 (Task 75): R88 horizon-check feed ported from KaggressurE V39 (EXP219) ----
...
# v14-M tail layers (from v13, Task 69 H1 merge): PREDUMP (disabled) + prefire h21/h22 + melon-seller.
...
# v14 FRONT-RUN (Task 69, H5): pre-empt the mirror opponent's scheduled premium dumps.
# The opponent is the same public lineage (Apache-2.0): its route choice is a pure
# function of the PUBLIC shop draw, so its tape is exactly knowable in advance.
...
# ============================================================================
# v15 LAYER — R90 SHED-ANIMAL WATCHDOG (Task 73). OUTERMOST LAYER.
# Root cause of the -$5,973 Kaggle loss to Dmitriy Ulybin (seed 661817856,
# Task 72 autopsy): a COW bought into a full pasture -> worker PICKUP ->
# PLACE failed -> the end-of-day inventory dump returned it to the shed ->
# the planner never retried; the animal sat in the shed 26 days while empty
# pastures stood available (reproduced locally on seed 300 seat 1, cow stuck
# t268->t718 after the native gave up retrying at d11).
#
# DESIGN (v2, after the seed-300 seat-1 incident taught two hard lessons:
# a pool-age grace clock taints newly bought animals with the pool's age and
# racing V233's two-day sheep placement flow is catastrophic; displacing the
# farmer breaks morning feeding and kills animals):
#   * ACTIVITY-BASED ORPHAN DETECTOR: an animal type is a rescue candidate
#     only after its grace period of COMPLETE QUIET — no shed count change
#     (buy/pickup/dump), no in-flight PICKUP/PLACE commands, no carried
#     units, no BUY_ANIMAL order. A healthy placement flow (V233's 6-sheep
#     two-day investment, native retries) is continuously active and is
#     never raced; an orphan goes quiet and fires.
#   * DEDICATED HIRE: the rescue runs on a freshly hired hand (cost ~$3-13,
#     spawns on a shed-access tile, dismissed at the day boundary) — the
#     farmer and the native's own workers are never displaced.
#   * Launch window hour<=6, never on days 11-12 (V233's day-12 exact hand
#     count check must not see an extra hand at its h1-h2 retry), never when
#     the placement can no longer pay before the day-28 production boundary
#     (COW<=20, SHEEP<=22, GOOSE<=24).
#   * Execution: the hand PICKUPs min(pool, empty sites, shed-room) and
#     PLACEs on the nearest empty matching structure tiles, one per turn,
#     re-checking sites each turn; missions are per-day (hands are dismissed
#     at the day boundary; the mission relaunches the next day if needed).
_R90_STRUCT={'GOOSE':'COOP','COW':'PASTURE','SHEEP':'PASTURE'}
_R90_GRACE={'COW':24,'SHEEP':30,'GOOSE':24}      # QUIET steps (activity resets)
...
def _r90_mission_step(obs,action,state,m):
    """Drive the dedicated rescue hand: PICKUP -> walk -> PLACE (one per turn)."""
    ...
    if not sites:
        home=min(_R90_ACCESS,key=lambda p:abs(hx-p[0])+abs(hy-p[1]))
        set_hand(_v219_walk(pos,home) or ['PASS'])   # wait near shed; EOD dump is safe
        return result
...
def _r90_watchdog(obs,action):
    ...
    # ---- activity-based quiet clock (the orphan detector) ----
    ...
    if m is not None:
        if m['day']!=day:
            state['mission']=None;m=None       # hands dismissed at the day boundary
        else:
    ...
    for a in ('COW','SHEEP','GOOSE'):
        ...
        # spawn estimate for the hired hand (engine: least-occupied access tile)
