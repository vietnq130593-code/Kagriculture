# v5 "ORCHESTRATOR" — P0 (PLAN_V5.md §7): nền kinh tế v4 + L5 Self-Assessment
# (sổ KPI đêm + điểm sức khỏe H) + L6 Risk Guard lõi F1/F2/F3 (playbook + lối thoát
# bắt buộc) + L2-lite flow rooms (E[opp flow] 3 đêm thay valve tĩnh) + endgame labor
# surge d26-28 + straw decay window + _arena_diag() xuất não cho Arena Observer UI.
# Mọi khác biệt vs v4.py đánh dấu "# v5:". Fallback: mọi lớp não hỏng → nền v4.
import math
...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money, tm=None, rg=None):
    ...
    # v5 L2-lite: room theo E[opp flow] 3 đêm (x1.3 thận trọng) khi CONTEST và đã có
    # dữ liệu — NHƯNG theo nguyên tắc minimax P0: chỉ chấp nhận ước lượng flow khi
    # NÓ THẬN TRỌNG HƠN valve tĩnh (lấy max phần trừ) → v5 không bao giờ ramp mạnh
    # hơn v4 trên cùng dữ liệu; phần exploit chủ động đợi P1 posterior đúng.
    # MIRROR giữ valve tĩnh của v4 (mirror-safety), thiếu dữ liệu → tĩnh.
            ...
            if kind == "PLANT":
                ...
                if yu > 0 and age >= cd["first_yield_day"]:
                    ...
                    if cd["ongoing"]:
                        ...
                        if False and not ready and yu >= 3 and mls >= 0 and mls - step <= 12:
                            ready = True
                    else:
...
def _build_orders(me, shed, seeds, inventories, inv, prices, day, hour, plan,
    ...
    if hour <= 8 and 1 <= day <= 22 and not (rg and int(rg.get("F1", 0) or 0) >= 2):  # v5: F1 L2 ngừng mua thú
    ...
    nq = len(unlocked)
    if nq < 4 and bought.get("LAND", 0) < 1 and not (rg and int(rg.get("F1", 0) or 0) >= 2):  # v5: F1 L2 hoãn mua đất
