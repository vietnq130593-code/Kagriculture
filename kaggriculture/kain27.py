...
#      pipeline ngỗng của ta → egg_room âm → rút về floor 3 → ta độc chiếm kênh.
#   Δ3 COW-SOFT: cow cap 4 (v6 đến 10) — NHƯỢNG kênh sữa cho v6 (phân bổ kênh
#      bất đối xứng: ta trứng+len+dâu, v6 sữa — tránh va chạm mirroring).
#   Δ4 QUOTA-FIT-75: straw 30→22, melon 14→12 — quota khớp diện tích 75 ô
#      (46 crop + 16 thú/structure = 72 ≤ 75).
#   Δ5 CARE-LOCK: SERVICE ngỗng tier 1 (trên harvest) — care-bank ngỗng trả
#      TIỀN HẰNG NGÀY (R93: care = 100% năng suất; probe: 47 vs 71 sữa).
#   Δ6 HERD-CAP 16 (từ 15): 8-9 ngỗng + 5-7 cừu + 1-4 bò.
# Cơ khí v6.6 giữ nguyên: melon collider, tranche-8, feed-gate $52, wheat-churn
# (R37), straw escalation (R96), water-CRIT premium-only (R91), pre-dump d27
# (R83), wheat-reserve ×3, E8-lite drain-aware hold d22-27.
#
# NHÂN QUẢ v6 (160 game battery): v5-chassis + delta KAIN = 52.5% vs v5 —
# twin-kernel ĐỐI XỨNG: cả hai đồng phục ~$57-59k (pháo đài R95 định lượng).
# Chỉ PROFILE NGOÀI kernel-space của v5 (kain16 = v4-lineage, CONTEST mode)
# mới thắng (93.75% / 80 game). v6 = chassis kain16 (93.75%) + các mảnh
# herd-engine của v5 đã chứng minh qua care-probe seed 123:
#   (REVERTED ΔA straw-ramp: pipeline dâu 30-từ-d5 là đòn yield của kain16)
#   (REVERTED ΔB geese: va chạm ngỗng-sớm 129/131/138 — floor-7 khi v5 cũng
#    floor-7 = kênh trứng sập cả hai; kain16 goose formula giữ nguyên)
#   ΔD FEED-TỚI-D26: dairy_deep d≤26 (từ 23) + acute pw≤64 d≥24 — đàn
#      không chết đói d24-28 (probe: feed-miss 11/15 thú d26)
# Giữ nguyên kain16: melon-14 collider (K13 idol), tranche-8 (R90/K16),
# feed-gate $52 (R92/K11), herd-cap 15 (R88/K11), wheat-churn engine (R37),
# straw escalation (R96/K10), water-CRIT premium-only (R91/K9),
# pre-dump d27 (R83/K15), wheat-reserve ×3 (K15).
# v5.py được bảo vệ nguyên vẹn (đối thủ chuẩn + đối chiếu A/B).
...
T_WATER_CRIT = 0
T_SERVICE_URG = 0
T_SERVICE_CARE = 1   # KAIN-25 Δ5: goose SERVICE (care-bank trả tiền hằng ngày)
T_HARVEST_URG = 2
T_DELIVER = 2
T_BUILD_URG = 2
T_SERVICE = 2
...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money, tm=None):
    ...
    # KAIN-25 Δ2/Δ3: egg-fortress (floor 6, cap 9, nhạy hơn) + cow-soft (cap 4)
    goose_target = max(6, min(9, int(egg_room // 40)))
    cow_target = max(1, min(4, int(milk_room // 30)))
    ...
    _herd_cap = 16 if (day >= 11 or money >= 3000) else 13   # KAIN-25 Δ6
    tot = cow_target + sheep_target + goose_target
    if tot > _herd_cap:
        over = tot - _herd_cap
        goose_target = max(3, goose_target - over)
        tot = cow_target + sheep_target + goose_target
        if tot > _herd_cap:
    ...
    # KAIN-26: melon 13 — giữ volume collider (v6 bán 108u vs ta 78u seed 123)
    if day <= 1:
        quotas["MELON"] = 13
    elif 2 <= day <= 7:
        quotas["MELON"] = 13
    elif 8 <= day <= 14:
        quotas["MELON"] = 13 if room("MELON") > -50 else 6
    if 2 <= day <= 16:
        quotas["TOMATO"] = 0
    if 5 <= day <= 13:
        ...
        want_straw = 26   # KAIN-26: straw 26 (kênh dâu KHÔNG bão hòa — seed 123: v6 80u/$23k vs ta 46u/$12.7k)
        ...
        # KAIN-27 FIX4: giá premium + kênh không glut -> quota đầy, bỏ phản hồi pipeline
        if spx >= 1.1 * MARKET_PARAMS["STRAWBERRY"]["base"] and sinv <= MARKET_I0:
            s_room = max(s_room, want_straw * 4 + 40)
    ...
    if day <= 24:
        if day <= 4:
            ...
            if (standing.get("STRAWBERRY", 0) < 26 and 6 <= day <= 16
                    and quotas.get("STRAWBERRY", 0) >= 24):
                quotas["WHEAT"] = min(quotas["WHEAT"], 14)
    if day <= 23:
        if day <= 2:
            quotas["CARROT"] = 12
        elif day <= 10:
        ...
        for x in range(board):
            ...
            if kind == "PLANT":
                ...
                if day <= 28 and (need_feed or need_care or need_fert):
                    _tier = (T_SERVICE_URG if starve else
                             (T_SERVICE_CARE if (t.get("animal") == "GOOSE"
                                                 and need_feed)   # KAIN-27 FIX2: tier 1 chỉ khi ĐÓI
                              else T_SERVICE))
                    tasks.append(_mk(_tier, x, y, "SERVICE",
                                     want_wheat=need_feed, feed=need_feed))
                yu = t.get("yield_units", 0) or 0
                if yu >= ad["max_held"] - 1:
                    tasks.append(_mk(T_HARVEST_URG, x, y, "HARVEST"))
                elif yu >= 3:
                    # KAIN-27 FIX1: yu>=3 tier 1 — chống kẹt max_held (probe 112:
                    # wool ta 24u vs v6 70u vì len ngồi trên tile khóa sản xuất)
                    tasks.append(_mk(T_SERVICE_CARE, x, y, "HARVEST"))
...
def _build_orders(me, shed, seeds, inventories, inv, prices, day, hour, plan,
    ...
    if nq < 3 and bought.get("LAND", 0) < 1:   # KAIN-25 Δ1: cap 75 ô, không mua SW $4k
        ...
        if day <= last_day and (day <= 1 or owned_empty <= 14 or money >= price * gate) \
    ...
    if animals > 0 or plan_animals > 0:
        ...
        if shed_wheat < wheat_want and money >= 400:
            ...
            _dairy_deep = (_pxm >= 200 or _pxw2 >= 220) and day <= 26   # KAIN-27 FIX3 feed-d26
            _tgt_unmet = (animals < (plan.get("cow_target", 0) + plan.get("sheep_target", 0)))
            if n >= 1 and (pw <= 38 or (acute and pw <= (64 if day >= 24 else 62))
