...
# ---------------------------------------------------------------------------
# t102 LATE-STRAW (v26): a second strawberry planting on the wheat rotation.
#
# The tape plants its 33 strawberries once (days 5-11); the plants finish by
# day 22-28 and the berry book runs dry for the last week while berry shops
# keep draining it -- day-24+ quotes sit at $108-$214 with no supply.  The
# wheat rotation meanwhile replants the same tiles every four days for a
# $37-$43 book that both farms glut.  From day 14-19 (so the plant gets
# three or four of its every-other-day picks in before the season ends),
# up to T102_STRAW_MAX PLANT WHEAT commands on just-harvested empty tiles
# are rewritten to PLANT STRAWBERRY; the recorded watering visits keep the
# plant alive (they water whatever stands on the tile), the recorded
# harvest visits collect the sitting yield (ongoing crops survive
# harvest), and later PLANT WHEAT / PLANT CARROT commands on a claimed
# tile are turned into PASS so the replant never overwrites the berry.
# Seeds are bought up front; surplus berries are sold like the CARROT
# layer sells its surplus carrots.
# ---------------------------------------------------------------------------
T102_STRAW_FIRST = 14
T102_STRAW_LAST = 19
T102_STRAW_RATIO = 3.0      # strawberry/wheat quote ratio needed to swap
T102_STRAW_MAX = 10         # claimed tiles
T102_STRAW_RESERVE = 70     # wheat cushion (shed + carried) before swapping
_T102_STRAW_REPORT = dict(t102_straw_tiles=0, t102_straw_plants=0,
                          t102_straw_blocked=0, t102_straw_harvests=0,
                          t102_straw_sold=0, t102_straw_errors=0)
_T102_STRAW = {}


def _t102_tile(farm, x, y):
    try:
...
def agent(observation, configuration=None):
    player, step = int(observation["player"]), int(observation["step"])
    day = step // 24
    st = _T102_STRAW.get(player)
    if st is None or step <= st["step"]:
        st = _T102_STRAW[player] = {"step": -1, "tiles": {}, "bought": False}
        if step == 0:
            _T102_STRAW_REPORT.update(t102_straw_tiles=0, t102_straw_plants=0,
                                      t102_straw_blocked=0, t102_straw_harvests=0,
                                      t102_straw_sold=0, t102_straw_errors=0)
    st["step"] = step
    action = _T102_STRAW_PARENT(observation, configuration)
    try:
        if not isinstance(action, dict):
            return action
        farm = observation["farms"][player]
        ...
        # drop claimed tiles whose plant is gone (harvested-out or dead)
        if st["tiles"]:
            alive = {}
            for (x, y), pd in st["tiles"].items():
                t = _t102_tile(farm, x, y)
                if isinstance(t, dict) and t.get("crop") == "STRAWBERRY":
                    alive[(x, y)] = pd
            st["tiles"] = alive

        # buy the seed stock on the first window day the gate is open
        if not st["bought"] and T102_STRAW_FIRST <= day <= T102_STRAW_LAST:
            ratio = (int(prices.get("STRAWBERRY", 0) or 0)) / max(1, int(prices.get("WHEAT", 99) or 99))
            wheat_held = int(private["shed"].get("WHEAT", 0) or 0) + \
                sum(int(i.get("WHEAT", 0) or 0) for i in private["inventories"])
            if ratio >= T102_STRAW_RATIO and len(market) < 10 and \
                    int(farm["money"]) > 1500:
                market.append(["BUY_SEED", "STRAWBERRY", T102_STRAW_MAX])
                st["bought"] = True
                changed = True

        seeds = int(private["seeds"].get("STRAWBERRY", 0) or 0)

        for i in range(min(len(commands), len(positions))):
            cmd = commands[i]
            if not cmd:
                continue
            op = cmd[0]
            ...
            p = (px, py)
            if op == "PLANT" and len(cmd) >= 2 and cmd[1] in ("WHEAT", "CARROT"):
                if p in st["tiles"]:
                    commands[i] = ["PASS"]
                    _T102_STRAW_REPORT["t102_straw_blocked"] += 1
                    changed = True
                elif (cmd[1] == "WHEAT" and T102_STRAW_FIRST <= day <= T102_STRAW_LAST
                        and len(st["tiles"]) < T102_STRAW_MAX and seeds > 0):
                    ratio = (int(prices.get("STRAWBERRY", 0) or 0)) / \
                        max(1, int(prices.get("WHEAT", 99) or 99))
                    wheat_held = int(private["shed"].get("WHEAT", 0) or 0) + \
                        sum(int(i.get("WHEAT", 0) or 0) for i in private["inventories"])
                    t = _t102_tile(farm, px, py)
                    if ratio >= T102_STRAW_RATIO and wheat_held >= T102_STRAW_RESERVE \
                            and t is None:
                        commands[i] = ["PLANT", "STRAWBERRY"]
                        st["tiles"][p] = day
                        _T102_STRAW_REPORT["t102_straw_plants"] += 1
                        changed = True
            elif op == "WATER" and p in st["tiles"]:
                t = _t102_tile(farm, px, py)
                if isinstance(t, dict) and t.get("crop") == "STRAWBERRY" \
                        and int(t.get("yield_units", 0) or 0) >= 1:
                    commands[i] = ["HARVEST"]
                    _T102_STRAW_REPORT["t102_straw_harvests"] += 1
                    changed = True
        _T102_STRAW_REPORT["t102_straw_tiles"] = len(st["tiles"])

        # sell surplus strawberries the way the carrot layer sells carrots
        if st["tiles"] and day < 29:
            result = dict(action)
            result["farmer"], result["hands"], result["market"] = commands[0], commands[1:], market
            stock = projected_shed(result, FarmView(observation)).get("STRAWBERRY", 0)
            selling = sum(int(o[2]) for o in market
                          if len(o) >= 3 and o[:2] == ["SELL", "STRAWBERRY"])
            if stock > selling and len(market) < 10 \
                    and int(prices.get("STRAWBERRY", 0) or 0) >= 2:
                market.insert(0, ["SELL", "STRAWBERRY", int(stock - selling)])
                _T102_STRAW_REPORT["t102_straw_sold"] += int(stock - selling)
                changed = True
        if changed:
            action = dict(action)
        ...
        _T102_STRAW_REPORT["t102_straw_errors"] += 1
        return action


agent.telemetry = _T102_STRAW_REPORT
agent = globals().pop('agent')
