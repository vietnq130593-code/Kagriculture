# ============================================================================
# V12 — "KME3-TUNED" (KAIN, Task 59): lõi = kme3 (Master Engine V3, Apache 2.0),
# tinh chỉnh duy nhất trên trục CADENCE-RACE sau bisection 8 biến thể (a-h):
#   T1 HORIZON=6 (kme3=4): mỗi lot bán trước kme3 2 turn suốt d12-28
#      → 16/16 thắng, gap +$2.4k, v12 $99.5k (Pareto: tiền tối đa + gap tối đa)
#   T4 WINDOW d29 (HI=712; kme3 dừng 696): kéo cadence-race vào cả ngày 29.
#   T2 EARLY-R36 (LO<288): đo lường = NO-OP (đàn hồi nửa đầu vôn-tưởng) — để 288.
#   T3 PREDUMP: NO-OP — shadow-planner kme3 đã bán "1000 mọi thứ" MỖI turn 712-717,
#      cuối game không còn hàng tồn để bán sớm hơn → tắt (-1).
# Đường cong horizon: H=5 +2.1k | H=6 +2.4k (đỉnh) | H=8 +2.4k | H=10 +1.7k |
#   H=16 +0.7k (tự hại: bỏ lỡ drain-driven price-rise 16 turn)
# Baseline: kme3 self-play = HÒA TUYỆT ĐỐI mọi seed (78-113k tuỳ seed, 2 ghế như nhau).
# (Hằng số _V12_* đặt sau `from __future__` — cú pháp Python bắt buộc.)
# ============================================================================
...
# ---------------------------------------------------------------- V12 config (FINAL: H=6, HI=712)
_V12_HORIZON = 6        # T1: R36 reservation horizon (kme3 = 4; đường cong đỉnh tại 6)
_V12_R36_LO = 288       # T2: R36 window start (kme3 = 288 = d12; LO=24 no-op)
_V12_R36_HI = 712       # T4: R36 window end exclusive (kme3 = 696; kéo sang d29)
_V12_PREDUMP_STEP = -1  # T3: -1 = tắt (kme3 dump 718) — shadow-planner đã bán
...
def agent(observation, configuration=None):
    ...
    state['step'] = step
    # V12-T2: horizon mở cho toàn bộ cửa sổ R36 (_V12_R36_LO.._V12_R36_HI),
    # ngoài cửa sổ giữ gốc 2 như kme3.
    ...
    # V12-T1: horizon nâng lên _V12_HORIZON (5) — vượt trước mọi lệnh R36 của kme3.
...
# ============================================================================
# V12-T3: TERMINAL PREDUMP — bán sạch shed 1 turn TRƯỚC khi kme3 dump (718).
# Giữ nguyên lệnh đơn vị của parent (shadow-planner 712-718 đã tối ưu đường
# đi/harvest), chỉ thay lệnh market tại _V12_PREDUMP_STEP bằng toàn bộ shed
# chiếu-tính, sắp theo giá trị giảm dần. Trong mirror: kme3 dồn hết hàng bán
# ở 718 vào giá đã sập sau đợt dump của v12. Với đối thủ khác: trung tính.
# ============================================================================
