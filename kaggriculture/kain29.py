# KAIN-29 "PROFILE-75" (Task 28 vòng 5): kain28 + profile v6 trên 75 ô (straw 30, melon 14, wheat 14 nhường ô dâu)
# (protocol §P: 1 delta/battery; kain27 multi-fix = regression 26/100... verify)
# = v6.6 + 6 DELTA CẤU TRÚC gốc:
...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money, tm=None):
    ...
    # KAIN-25 Δ4: melon 12 (từ 14) — collider giữ nhưng khớp 75 ô
    if day <= 1:
        quotas["MELON"] = 12
    elif 2 <= day <= 7:
        quotas["MELON"] = 12
    ...
    if 5 <= day <= 13:
        ...
        want_straw = 22   # KAIN-25 Δ4: straw 22 (từ 30) — khớp 75 ô
        ...
        # KAIN-29: giá premium + không glut -> quota đầy (bỏ phản hồi pipeline)
        ...
        for x in range(board):
            ...
            if kind == "PLANT":
                ...
                if day <= 28 and (need_feed or need_care or need_fert):
                    _tier = (T_SERVICE_URG if starve else
                             (T_SERVICE_CARE if t.get("animal") == "GOOSE"
                              else T_SERVICE))   # KAIN-25 Δ5 care-lock ngỗng
                ...
                if yu >= ad["max_held"] - 1:
                    ...
                    # KAIN-28: yu>=3 tier 1 — sản phẩm ngồi trên tile KHÓA sản xuất
                    # (probe 112: wool 24u vs 70u; seed 112 kain27+fix: 86u lật +$15k)
