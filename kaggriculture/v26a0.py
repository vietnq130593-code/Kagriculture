...
_Y_CFG={'yarnsheep': True, 'yarngeese': True, 'days': (8, 11),
        'nosheep': True, 'min_milk': 1, 'nogeese': True}
...
def _y_controller(obs,action,state,cfg):
    ...
    try:
        _v26_win = cfg['days'] if _v251_racing(seat) else (min(6, cfg['days'][0]), cfg['days'][1])
    except Exception:
        _v26_win = cfg['days']
        ...
        for off in range(1, _V26_HZ_MAX + 1):
            ...
            for o in _adv_future(player, t):
                ...
                if o[0] == 'SELL' and o[1] in _ADV_ITEMS:
                    ...
                    try:
                        if False and item in _V26_BASE and int(prices.get(item, 0)) <= _V26_BASE[item]:
