...
# Giữ nguyên toàn bộ phần còn lại của kain33 (4 luật đất cứng + plan-cache
# pop + mọi seam S1-S4/E8-lite).
...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money, tm=None):
    ...
    # Δ1 FILL-AFFORD (kain34): d4-14 — (a) rút ô từ cây đắt (dâu/dưa) khi
    # hạt không mua nổi hôm nay theo floor mua hạt, đổ sang wheat hạt $10;
    # (b) lấp TOÀN BỘ ô còn trống khi feed_demand cao (đàn ăn wheat + bán).
    if 4 <= day <= 14:
        try:
            for _c in ("STRAWBERRY", "MELON"):
                _take = crop_tiles.get(_c, 0)
                if _take <= 0:
                    continue
                _fl = 900 if _c == "STRAWBERRY" else 700
                _aff = max(0, int((money - _fl) // CROPS[_c]["seed"]))
                _over = _take - _aff
                if _over > 0:
                    crop_tiles[_c] = _take - _over
                    crop_tiles["WHEAT"] = crop_tiles.get("WHEAT", 0) + _over
            if remaining > 0:
                if feed_demand > 200:
                    crop_tiles["WHEAT"] = crop_tiles.get("WHEAT", 0) + remaining
                else:
                    crop_tiles["CARROT"] = crop_tiles.get("CARROT", 0) + remaining
                remaining = 0
        except Exception:
            pass
    ...
    if hour <= 17 and seeds is not None:
        for crop in ("WHEAT", "CARROT", "STRAWBERRY", "MELON", "TOMATO"):
            ...
            if have < n_tiles:
                ...
                if crop == "WHEAT" and n_tiles > 24:
                    # Δ1: fill-wheat — hạt $10 quay vòng 5 ngày, KHÔNG giữ
                    # land_reserve (autopsy d5-9: reserve 1150 chặn luôn hạt rẻ)
                    floor = 60
