# v26c 'HERD-ADAPTIVE ANSWER GENERAL' (Task 100, 19-09) = v25.1 + port ĐÚNG
# verdict COW của 2945-farm V9 herd (thomastschinkel 'The 2945 Farm v9/4'):
# khi mua SHEEP/GOOSE trong window (8,11): egg_shops==0 AND YARN chưa mở AND
# milk_shops>=3 AND MILK>=$150 -> đổi thành COW ('bò chuyên sâu', profile 12-bò
# của 2945 trên s110, +$13.5k/ghế theo đối chứng battery). Gate CHÍNH XÁC theo
# V9_HERD_MAX_EGG_SHOPS_COW=0 / V9_HERD_MIN_MILK_SHOPS=3 / V9_HERD_MIN_MILK=150
# — không đánh cược YARN tương lai (bản port rộng v26 đầu đã thua s102-104
# -8.7k vì swap sớm khi YARN mở muộn; mổ xẻ bench/t100_*). Mọi cấu hình shop
# khác: hành vi GIỮ NGUYÊN v25.1 (đối chứng byte-exact s100/s101 vs v48/
# alperen1). KHÔNG thêm lớp runtime nào (hz40/ORDERPRI2/CAPHARV/glut-gate đã
# loại sau ablation A0/A3: neutral-trên-s102-104, -2.3k trên s122).
...
# Task 100 (v26c) — flag bật verdict COW của 2945-farm V9 herd trong _y_target
...
def _y_target(kind,shops,cfg,prices=None):
    ...
    # Task 100 (v26c) — port verdict COW của 2945-farm V9 herd với gate
    # CHÍNH XÁC V9_HERD_*: egg_shops==0 (MAX_EGG_SHOPS_COW) AND YARN chưa mở
    # AND milk_shops>=3 (MIN_MILK_SHOPS) AND MILK>=150 (MIN_MILK) -> SHEEP/
    # GOOSE mua trong window (8,11) thành COW. Không đánh cược YARN tương
    # lai: cấu hình khác KHÔNG verdict -> fall-through giữ nguyên rules
    # v25.1 bên dưới (đối chứng byte-exact đã chứng minh).
