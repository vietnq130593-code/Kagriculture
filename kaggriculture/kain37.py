...
# Giữ: Δ1'/Δ1'' straw-partial + carrot fill, Δ2 PLANT tier-4, Δ3 HIRE-BOOST,
# Δ4 carrot d24, Δ5 late-straw d16-19, 4 luật đất cứng.
...
def _build_orders(me, shed, seeds, inventories, inv, prices, day, hour, plan,
    ...
    if nq < 3 and bought.get("LAND", 0) < 1:
        if nq == 1:
            if (5 <= day <= 7 or 8 <= day <= 9) and money >= 1000 + 350:
            ...
            if (10 <= day <= 12 or 13 <= day <= 14) and money >= 2000 + 400:
                orders.append(["BUY_LAND"])
                bought["LAND"] = 1
                money -= 2000
                _STATE.pop(("plan", day), None)
    ...
    if hour <= 17 and seeds is not None:
        for crop in ("WHEAT", "CARROT", "STRAWBERRY", "MELON", "TOMATO"):
            ...
            if have < n_tiles:
                ...
                floor = (30 if crop == "WHEAT" else
                         (150 + land_reserve) if crop == "STRAWBERRY" else (220 if crop == "CARROT" else 220 + land_reserve))
                if crop == "STRAWBERRY":
                    floor = 900
