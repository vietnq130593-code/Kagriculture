# v6 "ORCHESTRATOR-K" — v6.1 (Task 27, 23 Sep): v5.6+E + 6 DELTA KAIN (R88–R96).
# v6 = nền tảng v5.6+E (97% vs v4, 99% vs v3 — KHÔNG đổi kernel) + vũ khí
# zero-sum của KAIN đã thắng v5 93.75% trên kernel v4 (kain16, 24 biến thể):
#   Δ1 TRANCHE-8 (R90/K16): MILK/WOOL/EGG bán 8/giờ đón giá phục hồi —
#      kênh drain-sâu ≥8u/ng; MELON drain 1u/ng vẫn dump toàn bộ.
#   Δ2 FEED-DAIRY-GATE (R92/K11): mua cám $52 khi sữa ≥$200/len ≥$220 +
#      target chưa đạt (đàn = máy quy đổi lúa mì→sữa 0.2×); want ×2.0.
#   Δ3 HERD-CAPITAL-CAP (R88/K4b): d≤10 cap target theo tiền mặt
#      (cow ≤ 1+money//900, sheep ≤ 2+money//1500) — mua đàn là RACE VỐN.
#   Δ4 BUY-PACE d14 (K8): pace 2 con/ngày đến d14 (v5: d12).
#   Δ6 SCARCITY-CAPITAL-GATE (R96/K22): floor hạt dâu 200 chỉ khi vốn
#      ≥$1.5k hoặc d≥12 — nghèo mua dâu = tự hủy.
# v6.1 PROFILE-SHIFT (bài học battery v1: 21/40 = pháo đài ĐỐI XỨNG —
#   twin-kernel đồng phục $57k; chỉ PROFILE KHÁC mới phá vỡ symmetry):
#   Δ7 STRAW-30 TỪ d5 (R83/R89 — K2/K10): first-mover kênh dâu $114/action,
#   front-run 3 ngày trước sóng dâu của v5.
#   Δ8 HERD-CAP 15 DEEP (R93/K11): d≥12 + deep_herd → cap u_est+2 —
#      care+tranche trả công đàn 15 (+$8-12k/game).
#   Δ9 PRE-DUMP d27 (R83/K15): bán trước cú dump d28 của v5 — zero-sum.
#   BỎ Δ3 capital-cap + Δ5 reserve-3 (battery v1: đồng phục với v5 = hại).
# Bỏ (bài học K13/R93): melon-14 collider; giữ melon-8 + Wave E của v5.
# v5.5 (Task 21): v5.4 + GT-LAND COURNOT + GT 2 CẤP (macro/micro).
...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money, tm=None, rg=None, n_units=0):
    ...
    if day < 2:
        ...
        sheep_target = 0
    # (v6 v2: BỎ Δ3 capital-cap — v5 đã có animal_reserve + cash_floor +
    # F-gates; cap thêm chỉ làm v6 nhỏ hơn v5 đúng cuộc đua sữa d6-10,
    # battery v1 chứng minh: v6−v5 đồng phục $57k = pháo đài đối xứng)
    ...
    # v5.6 Wave B: cap nới MUỘN (d≥18) — thời điểm nhàn trồng (quota đóng,
    # chỉ còn wheat watering + SERVICE). 15 thú @ 13 thợ = 45 SERVICE + ~60
    # tưới/thu trong 273 actions — dư dả. Autopsy 119: v4 đúng 15 thú @ d19
    # thắng $6.6k; bài học seed 42 (15 thú @ d8-13 giết trồng) KHÔNG áp dụng
    ...
    try:
        if V56_DEEP_HERD and 10 <= day < 18 and (tm or {}).get("deep_herd"):
            # v6 Δ8 (R93/R88 — KAIN K11): deep + care-discipline + tranche (Δ1)
            # trả công đàn 15 từ d12 (kain16 15 thú thắng $8-12k/game); d10-11
            # giữ ramp 13 tránh quá tải lao động sớm (bài học seed 42)
            _v6bump = 2 if day >= 12 else 0
            animal_cap = max(animal_cap, min(ANIMAL_CAP, u_est + _v6bump))
    except Exception:
        pass
    ...
    if 5 <= day <= 14:
        ...
        want_straw = 30 if day >= 5 else 14  # v6 Δ7 (R83/R89 — KAIN K2/K10): straw-30 từ d5
    ...
    # ---- v5.6 Wave A: LATE-ENGINE $/action + T/2 ----
    # P1 autopsy: 6/6 trận thua v5 dẫn giữa game (+$2.5k..+$13.7k @ d14-18)
    # rồi chảy máu d19-29 vì mọi quota đóng hết ≤d24 → ruộng rỗng @d27
    # (standing c2-c25 vs v4 c8-c21) trong khi v5 có 15 thợ (v4 13) = 200+
    # idle actions/ngày. Wheat/carrot cycle 4-5 ngày vẫn realizable (trồng d
...
def _build_orders(me, shed, seeds, inventories, inv, prices, day, hour, plan,
    ...
    if hour <= 8 and 1 <= day <= 22 and not near_land \
        ...
        for animal, target, w0, w1 in (("GOOSE", plan.get("goose_target", 0), 2, 14),
            ...
            # v5.6 Wave B: nới cửa mua cuối cho kênh deep (shop cấu trúc xác nhận
            # chiều sâu): bò d18/cừu d17 vẫn kịp 2-4 lứa sữa/len × giá cao.
            try:
            ...
            ad = ANIMALS[animal]
            cash_floor = 250
            buy_per_day = 2 if (day <= 14) else 1  # v6 Δ4 (K8): pace 2/ng đến d14
                ...
                if crop == "STRAWBERRY":
                    ...
                    try:
                        if V56_SCARCITY and (
                            ...
                            # v6 Δ6 (R96 — KAIN K22): nghèo mua dâu = tự hủy
                            # (101s0 cũ −$26k; K22 36/40) — floor 200 chỉ khi
                            # vốn đã có ≥$1.5k hoặc đã qua cửa vốn d12
                            if money >= 1500 or day >= 12:
                                floor = 200
                    except Exception:
                        pass
                if crop == "MELON":
    ...
    if animals > 0 or plan_animals > 0:
        ...
        # v6 Δ2a (R92 — KAIN K11): sữa sâu → đàn là máy quy đổi lúa mì
        # ($42 → sữa $265); x2 reserve giữ nhịp giao đàn khi kênh trả đủ
        try:
        ...
        if shed_wheat < wheat_want and money >= 400:
            ...
            if f2v >= 2 and pw > 0:
                ...
                n = min(need, afford)
            # v6 Δ2b (R92 — KAIN K11): mua cám đắt ($52) chỉ khi kênh sữa/len
            # sâu trả đủ (sữa ≥$200 / len ≥$220, d≤23) VÀ target đàn chưa đạt —
            # cổng $38 của v4 chặn mở rộng đàn khi lúa mì $40+ (K11: 33→37/40)
            try:
                _dairy_deep = ((float(prices.get("MILK", 0) or 0) >= 200)
                               or (float(prices.get("WOOL", 0) or 0) >= 220)) and day <= 23
                _tgt_unmet = animals < ((plan.get("cow_target", 0) or 0)
                                        + (plan.get("sheep_target", 0) or 0))
            except Exception:
                _dairy_deep = False
                _tgt_unmet = False
    ...
    # (v6 v2: BỎ Δ5 reserve ×3 — giữ nhịp bán wheat của v5 (vốn cho dâu sớm Δ7);
    # ×3 giữ $1-2k cám thêm = chậm dòng tiền hạt)
    ...
    def _hold(it):
        ...
        if day >= 28 - dliq:
            return 0.004
        # v6 Δ9 (R83/R90 — KAIN K15): PRE-DUMP d27 — bán TRƯỚC cú dump d28
        # của v5 (front-run cú sập cuối mùa — đòn zero-sum đúng nghĩa)
        if day >= 27:
            return min(h, 0.20)
        if day >= 26 - dliq:
            return min(h, 0.45)
    ...
    if shed:
        for it in PRODUCTS:
            ...
            if k > 0:
                # v6 Δ1 (R90 — KAIN K16): kênh drain-sâu (MILK/WOOL/EGG ~11-14u/ng)
                # — giá phục hồi giữa các giờ khi town drain hút lại tồn kho;
                # tranche 8/giờ thu premium thay vì dump 19 một lượt trượt
                # sàn ($265→$210). Kênh drain-cạn (MELON 1u/ng) giữ dump toàn
                # bộ (giữ = mất). K16: 37→38/40 + ~$1k/trận.
