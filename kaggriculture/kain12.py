...
def _build_orders(me, shed, seeds, inventories, inv, prices, day, hour, plan,
    ...
    if hour <= 8 and 1 <= day <= 22:
        ...
        milk_shops_n = sum(1 for s in (shops or [])
                           if s in ("PIZZA_SHOP", "ICE_CREAM_SHOP", "SMOOTHIE_SHOP"))
        yarn_n = sum(1 for s in (shops or []) if s == "YARN_STORE")
        _sheep_first = (4 <= day <= 10 and pxw >= 230)
        _order = (("SHEEP", plan.get("sheep_target", 0), 4,
            ...
            (("GOOSE", plan.get("goose_target", 0), 2,
             ...
             ("COW", plan.get("cow_target", 0), 5,
              16 + (2 if pxm >= 200 else 0) + (2 if milk_shops_n >= 2 else 0)))
        for animal, target, w0, w1 in _order:
    ...
    if animals > 0 or plan_animals > 0:
        ...
        if shed_wheat < wheat_want and money >= 400:
            ...
            # KAIN-12b: evening dairy sales defer cash — count the shed's
            # pending dairy value so the 35% feed budget isn't starved.
            try:
                _sdv = (float(shed.get("MILK", 0) or 0) * 200.0
                        + float(shed.get("WOOL", 0) or 0) * 180.0
                        + float(shed.get("EGG", 0) or 0) * 40.0)
            except Exception:
                _sdv = 0.0
            afford = int((money * 0.35 + min(_sdv, 3000.0)) // pw) if pw > 0 else 0
            n = min(need, afford)
            acute = shed_wheat < 6
    ...
    # KAIN-8: keep 4 days of feed in the shed — stop the sell-at-h0 /
    # buy-back churn (burned ~$25k/game vs v5's self-fed herd).
    ...
    cands = []
    if shed:
        # KAIN-12: hold MILK/WOOL/EGG until h>=20 — the day's drain bottomed
        # the inventory so the evening price is the day's peak (v5 sells its
        # dairy at h22-23; kain was selling at h0 into the overnight price).
        _dairy_gate = (hour < 20 and day < 26)
        for it in PRODUCTS:
            if _dairy_gate and it in ("MILK", "WOOL", "EGG"):
                try:
                    _px = float(prices.get(it, 0) or 0)
                except Exception:
                    _px = 0.0
                if _px < 1.4 * MARKET_PARAMS[it]["base"]:
                    continue
