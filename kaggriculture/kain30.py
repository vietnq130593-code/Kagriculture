# kain30 "SOLVER-1" (Task 31) — nghiên cứu hướng v7: v6.6 chassis + P3-LITE
...
def _daily_plan(tiles, shed, seeds, inv, prices, shops, day, opp_farm, money, tm=None):
    ...
    # S1 EGG-FORTRESS (R102): v6 hard-cap 6 ngỗng — kênh trứng KHÔNG thể
    # bị counter-scale. Khi đối thủ ≤ 7 ngỗng, floor 8 là đòn dương kỳ vọng.
    if opp_counts["GOOSE"] <= 7 and day >= 4:
    ...
    if tot > _herd_cap:
        # S1: bảo vệ pháo đài ngỗng — cắt BÒ trước, cừu sau (v6 cắt ngỗng trước)
        over = tot - _herd_cap
        cow_target = max(2, cow_target - over)
        tot = cow_target + sheep_target + goose_target
        if tot > _herd_cap:
...
def _agent(obs):
    ...
    pkey = ("plan", day, 1 if hour >= 12 else 0)
